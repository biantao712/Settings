...
public class SetupWizardUtils {
    ...
    public static void setImmersiveMode(Activity activity) {
        final boolean useImmersiveMode = activity.getIntent().getBooleanExtra(
                WizardManagerHelper.EXTRA_USE_IMMERSIVE_MODE, true /*Fix TT-903310*/);
        if (useImmersiveMode) {
    ...
    public static void copySetupExtras(Intent fromIntent, Intent toIntent) {
        ...
        toIntent.putExtra(WizardManagerHelper.EXTRA_USE_IMMERSIVE_MODE,
                fromIntent.getBooleanExtra(WizardManagerHelper.EXTRA_USE_IMMERSIVE_MODE, true /*Fix TT-903310*/));
    }

    // ++ For Asus style
    public static int getAsusTheme(Intent intent) {
        if (WizardManagerHelper.isLightTheme(intent, true)) {
            return R.style.AsusSetupWizardTheme_Light;
        } else {
            return R.style.AsusSetupWizardTheme;
        }
    }

    public static int getGoogleShortTheme(Intent intent) {
        if (WizardManagerHelper.isLightTheme(intent, true)) {
            return R.style.DefaultSetupWizardTheme_Light;
        } else {
            return R.style.DefaultSetupWizardTheme;
        }
    }
    // -- For Asus style
}
