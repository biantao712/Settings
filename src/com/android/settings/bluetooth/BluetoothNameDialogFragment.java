...
public final class BluetoothNameDialogFragment extends DialogFragment implements TextWatcher {
    ...
    private View createDialogView(String deviceName) {
        ...
        title.setText(getActivity().getResources().getString(R.string.bluetooth_rename_device));

        mDeviceNameView = (EditText) view.getRootView().findViewById(R.id.alertdialog_textfield);
        mDeviceNameView.setFilters(new InputFilter[] {new Utf8ByteLengthFilter(248), new RemoveSpecialCharFilter(false)});
        mDeviceNameView.setText(deviceName, TextView.BufferType.EDITABLE);
        mDeviceNameView.setSelection(deviceName.length());
        mDeviceNameView.setPadding(0,0,0,0);

        //mDeviceNameView = (EditText) view.findViewById(R.id.edittext);
   /*     mDeviceNameView.setFilters(new InputFilter[] {new Utf8ByteLengthFilter(BLUETOOTH_NAME_MAX_LENGTH_BYTES), new RemoveSpecialCharFilter(false)});
    ...
    public void onResume() {
        ...
        if (mOkButton == null) {
            ...
            mOkButton.setEnabled(/*mDeviceNameEdited && */(mDeviceNameView.getText().toString().trim().length() != 0));    // Ok button enabled after user edits
        }
