...
final class BluetoothDiscoverableEnabler implements Preference.OnPreferenceClickListener {
    ...
    private final SwitchPreference mDiscoveryPreference;
    ...
    BluetoothDiscoverableEnabler(LocalBluetoothAdapter adapter,
            SwitchPreference discoveryPreference) {
    ...
    public void resume(Context context) {
        ...
        //Suleman
        boolean mIsOn;
        mIsOn = LocalBluetoothPreferences.getSwitchIsOn(mContext);
       // mDiscoveryPreference.setChecked(mIsOn);
        Log.d(TAG,"mLocalAdapter.getScanMode()= " + mLocalAdapter.getScanMode());
   
       if(BluetoothAdapter.SCAN_MODE_CONNECTABLE_DISCOVERABLE == mLocalAdapter.getScanMode()) {
           mDiscoveryPreference.setChecked(true);
       } else {
           mDiscoveryPreference.setChecked(false);
       }
    ...
    public boolean onPreferenceClick(Preference preference) {
        ...
        //+++ suleman
        if(mDiscoveryPreference.isChecked()) {
            LocalBluetoothPreferences.saveSwitchIsOn(mContext,true);
            mDiscoverable = true;
        } else {
            LocalBluetoothPreferences.saveSwitchIsOn(mContext,false);
            mDiscoverable = false;
        }
        //--- suleman
    ...
    void setDiscoverableTimeout(int index) {
        ...
        LocalBluetoothPreferences.setCheckTime(mContext,timeoutValue);
    ...
    private int getDiscoverableTimeout() {
        ...
        if (timeout < 0) {
           ...
           //         VALUE_DISCOVERABLE_TIMEOUT_TWO_MINUTES);
            ...
            String timeoutValue = LocalBluetoothPreferences.getCheckTime(mContext,VALUE_DISCOVERABLE_TIMEOUT_TWO_MINUTES);
