...
public abstract class DeviceListPreferenceFragment extends
    ...
    void addCachedDevices() {
        ...
        if (mDeviceListGroup == null) return;
        int count = mDeviceListGroup.getPreferenceCount();
        for (int i = 0; i < count-1; i++) {
            Preference pref = mDeviceListGroup.getPreference(i);
            if (pref instanceof BluetoothDevicePreference) {
                BluetoothDevicePreference pairedPref = (BluetoothDevicePreference) pref;
                pairedPref.setLayoutResource(R.layout.preference_bt_icon);
            }
        }
        if (count > 0) {
            Preference pref = mDeviceListGroup.getPreference(count-1);
                if (pref instanceof BluetoothDevicePreference) {
                    BluetoothDevicePreference pairedPref = (BluetoothDevicePreference) pref;
                    pairedPref.setLayoutResource(R.layout.preference_bt_icon_no_divider);
                }
        }
    }
    ...
    void createDevicePreference(CachedBluetoothDevice cachedDevice) {
        ...
        mDeviceListGroup.setOrderingAsAdded(true);
