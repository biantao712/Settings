...
import com.asus.bluetooth.BluetoothPairedUtil;
...
public final class BluetoothSettings extends DeviceListPreferenceFragment implements Indexable {
    ...
    private static final int MENU_ID_VERIZON_HELP  = Menu.FIRST + 3;

    private static final int BUTTON_ID_REFRESH  = 1;
    private static final int BUTTON_ID_CHECK_TIME  = 2;

    private static final int MENU_ID_VISIBILITY_TIMEOUT = Menu.FIRST + 4;

    private static final String BT_ASUS_ADD_CATEGORY = "bt_asus_add_category"; 
    private static final String BT_SWITCH_PREF = "bt_switch_pref";
    private static final String BT_CHECK_PREF = "bt_check_pref";
    private static final String BT_RENAME_PREF = "bt_rename_pref";
    private static final String BT_RECEIVE_PREF = "bt_receive_file_pref";
    ...
    private PreferenceGroup mAsusBtDockDevicesCategory;
    ...
    //Suleman
    private PreferenceGroup mAsusAddCategory;   
    private SwitchPreference mBtSwitchPreference;
    private SwitchPreference mBtCheckPreference;
    private Preference mMyDeviceNamePreference;
    private Preference mReciveFilePreference;
    ...
    private final BroadcastReceiver mReceiver = new BroadcastReceiver() {
        ...
        private void updateDeviceName(Context context) {
            if (mLocalAdapter.isEnabled() && mMyDeviceNamePreference != null) {
                ...
                */
                //+++ suleman
                mMyDeviceNamePreference.setSummary(
                            bidiFormatter.unicodeWrap(mLocalAdapter.getName()));
                //---
            }
        }
    ...
    public void onActivityCreated(Bundle savedInstanceState) {
        ...
        mAsusAddCategory = (PreferenceCategory) findPreference(BT_ASUS_ADD_CATEGORY);
        mBtSwitchPreference = (SwitchPreference) findPreference(BT_SWITCH_PREF);
        mBtCheckPreference = (SwitchPreference) findPreference(BT_CHECK_PREF);        
        mMyDeviceNamePreference = (Preference) findPreference(BT_RENAME_PREF);
        mReciveFilePreference = (Preference) findPreference(BT_RECEIVE_PREF);

        final SettingsActivity activity = (SettingsActivity) getActivity();
        mBluetoothEnabler = new BluetoothEnabler(activity, mBtSwitchPreference);
       ...
       // }
        ...
        buttonBar = activity.getButtonBar();
        if(buttonBar != null) {
            buttonBar.setVisibility(View.VISIBLE);
            buttonBar.addButton(BUTTON_ID_REFRESH, R.drawable.bluetooth_refresh_button, res.getString(R.string.bluetooth_refresh_button_title));
            buttonBar.getButton(BUTTON_ID_REFRESH).setOnClickListener(new View.OnClickListener() {
                @Override
            ...
            buttonBar.addButton(BUTTON_ID_CHECK_TIME, R.drawable.bluetooth_check_time_button, res.getString(R.string.bluetooth_check_time_button_title));
            buttonBar.getButton(BUTTON_ID_CHECK_TIME).setOnClickListener(new View.OnClickListener() {
                @Override
                public void onClick(View v) {
                if (DiscoverableEnablerManager.getInstance(getActivity()).getDiscoverableEnabler() == null) 
                    return ;
        ...
        setButtonEnabled(bluetoothIsEnabled);
    }
    ...
    public void onDestroyView() {
        ...
        DiscoverableEnablerManager.getInstance(getActivity()).setDiscoverableEnabler(null);
        //
    ...
    public void onResume() {
        ...
        if (mBluetoothEnabler != null) {
	        ...
	        if(VerizonHelpUtils.isVerizonMachine()){            	
            	  mBluetoothEnabler.setupTutorial(getActivity());            	
	        }
            // ---
        }
        super.onResume();
    ...
    public boolean onPreferenceTreeClick(Preference preference) {
        if (preference == mBtSwitchPreference){
            Log.i(TAG, " mBtSwitchPreference ="+ mBtSwitchPreference.isChecked());
        }else if (preference == mBtCheckPreference){
            if (DiscoverableEnablerManager.getInstance(getActivity()).getDiscoverableEnabler() == null) return true;
                new BluetoothVisibilityTimeoutFragment().show(
                        getFragmentManager(), "visibility timeout");
            Log.i(TAG, " mBtCheckPreference = "+ mBtCheckPreference.isChecked());
        }else if (preference == mMyDeviceNamePreference){
            MetricsLogger.action(getActivity(), MetricsEvent.ACTION_BLUETOOTH_RENAME);
                if (getActivity()!=null)
                new BluetoothNameDialogFragment().show(
                        getFragmentManager(), "rename device");
            Log.i(TAG, " mMyDeviceNamePreference = ");
        }else if (preference == mReciveFilePreference) {
            MetricsLogger.action(getActivity(), MetricsEvent.ACTION_BLUETOOTH_FILES);
                Intent intent = new Intent(BTOPP_ACTION_OPEN_RECEIVED_FILES);
                getActivity().sendBroadcast(intent);
                Log.i(TAG, " mReciveFilePreference = ");
        }
        return super.onPreferenceTreeClick(preference);
    }

    public void setButtonEnabled(boolean enabled) {
        buttonBar.getButton(BUTTON_ID_CHECK_TIME).setEnabled(enabled);
        buttonBar.getButton(BUTTON_ID_REFRESH).setEnabled(enabled);
    }
    ...
    public boolean onOptionsItemSelected(MenuItem item) {
        switch (item.getItemId()) {
            ...
            case MENU_ID_VERIZON_HELP:
            	VerizonHelpUtils.launchVzWHelp(getActivity(), VerizonHelpUtils.SCREEN_BLUETOOTH);
                return true;
            // ---
        ...
        switch (bluetoothState) {
            case BluetoothAdapter.STATE_ON:
                ...
                if (getPreferenceScreen().getPreferenceCount() == 0) {
                    //Suleman
                    getPreferenceScreen().addPreference(mAsusAddCategory);
                    //
                    getPreferenceScreen().addPreference(mPairedDevicesCategory);
                    getPreferenceScreen().addPreference(mAvailableDevicesCategory);
                    //+++ suleman
                ...
                //Suleman
//                mBtCheckPreference.setEnabled(true);
//                mMyDeviceNamePreference.setEnabled(true);
//                mReciveFilePreference.setEnabled(true);
                //
        ...
        //        if (isUiRestricted()) {
                    if (mDiscoverableEnabler == null) {
                        mDiscoverableEnabler = new BluetoothDiscoverableEnabler(
                                mLocalAdapter, (SwitchPreference)mBtCheckPreference);
                        mDiscoverableEnabler.resume(getActivity());
                        DiscoverableEnablerManager.getInstance(getActivity()).setDiscoverableEnabler(
                                mDiscoverableEnabler);
                    }
        //        }
                //
                if (BluetoothPairedUtil.supportToPairWithBtDock()) {
                    if (mAsusBtDockDevicesCategory == null) {
                        mAsusBtDockDevicesCategory = new PreferenceCategory(getActivity());
                    } else {
                        mAsusBtDockDevicesCategory.removeAll();
                    }
                    getPreferenceScreen().addPreference(mAsusBtDockDevicesCategory);
                    addDeviceCategory(mAsusBtDockDevicesCategory,
                            R.string.bluetooth_preference_asus_bt_dock_devices,
                            BluetoothDeviceFilter.ASUS_BT_DOCK_DEVICE_FILTER, true);
                    int numberOfAsusBtDockDevices = mAsusBtDockDevicesCategory.getPreferenceCount();

                    if (isUiRestricted() || numberOfAsusBtDockDevices <= 0) {
                        preferenceScreen.removePreference(mAsusBtDockDevicesCategory);
                    }
                }
               ...
               //     mLocalAdapter.setScanMode(BluetoothAdapter.SCAN_MODE_CONNECTABLE_DISCOVERABLE); //Suleman
    ...
    public void onDeviceBondStateChanged(CachedBluetoothDevice cachedDevice, int bondState) {
        ...
        if (bondState == BluetoothDevice.BOND_BONDED) {
	       if(VerizonHelpUtils.isVerizonMachine()){
		     VerizonHelpUtils.DisplayTutorial(getContext(), VerizonHelpUtils.TUTORIAL_BLUETOOTH, VerizonHelpUtils.STEP_4, null);
	       }         
        }
        // ---
    }
...
*/
            ...
            final CachedBluetoothDevice device = (CachedBluetoothDevice) v.getTag();
            Bundle args = new Bundle();
            args.putString(PairedBluetoothDevice.ARG_DEVICE_ADDRESS,
                    device.getDevice().getAddress());
            final SettingsActivity activity =
                            (SettingsActivity) BluetoothSettings.this.getActivity();
                    activity.startPreferencePanel(PairedBluetoothDevice.class.getName(), args,
                           R.string.bluetooth_preference_paired_devices, null, null, 0);
        }
    };
    ...
    public static final SearchIndexProvider SEARCH_INDEX_DATA_PROVIDER =
        new BaseSearchIndexProvider() {
            ...
            public List<SearchIndexableResource> getXmlResourcesToIndex(Context context,
                ...
                indexable.xmlResId = R.xml.bluetooth_settings;
