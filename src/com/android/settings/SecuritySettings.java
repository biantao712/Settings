...
import com.asus.settings.lockscreen.ui.AsusOwnerInfoSettings;
...
public class SecuritySettings extends SettingsPreferenceFragment
    ...
    //++ Asus Verizon request
    private static final String KEY_SECURE_START_UP = "secure_start_up";
    private static final int ENABLE_ENCRYPTION_REQUEST = 2001;
    private static final int ENABLE_ENCRYPTION_REQUEST_VERIFY = 2002;
    private int mPwdType = -1;
    private String mPwd = null;
    //-- Asus Verizon request

    private static final String KEY_CRYPT_KEEPER_ENCRYPT_TITLE = "crypt_keeper_encrypt_title";
    ...
    private CNRestrictedSwitchPreference mToggleAppInstallation;
    ...
    private PreferenceScreen createPreferenceHierarchy() {
        ...
        Log.d(TAG,"add prefernece security_settings");
        ...
        */
        ...
        if (mProfileChallengeUserId != UserHandle.USER_NULL
            ...
            Log.d(TAG,"add prefernece security_settings_profile");
            Log.d(TAG,"add prefernece security_settings_unifications");
            ...
            Log.d(TAG,"add prefernece by ID : " + profileResid);
        ...
        if (mIsAdmin) {
            if (LockPatternUtils.isDeviceEncryptionEnabled()) {
                ...
                Log.d(TAG,"add prefernece security_settings_encrypted");
                addPreferencesFromResource(R.xml.security_settings_encrypted);
            } else {
                // This device supports encryption but isn't encrypted.
                Log.d(TAG,"add prefernece security_settings_unencrypted");
        ...
        if (securityCategory != null) {
            ...
            //remove fingerprint enrollment for Demo
            if(LiveDemoUnit.AccFlagRead()==0) {
                maybeAddFingerprintPreference(securityCategory, UserHandle.myUserId());
            }
            addTrustAgentSettings(securityCategory);
            addSecureStartUp(securityCategory);
            //--- suleman
        ...
        Log.d(TAG,"add prefernece security_settings_misc");
        ...
        if (!RestrictedLockUtils.hasBaseUserRestriction(getActivity(),
            ...
            final int storageSummaryRes =
                    mKeyStore.isHardwareBacked() ? R.string.security_credential_storage_type_hardware
        ...
        mToggleAppInstallation = (CNRestrictedSwitchPreference) findPreference(
                KEY_TOGGLE_INSTALL_APPLICATIONS);
        mToggleAppInstallation.setLayoutResource(R.layout.cn_restricted_switch_preference_nodivider);
        ...
        if (advancedCategory != null) {
            ...
            //++ Asus: Verizon request
            Preference manageSecureStartUp = advancedCategory.findPreference(KEY_SECURE_START_UP);
            if (manageSecureStartUp != null && !mLockPatternUtils.isSecure(MY_USER_ID)) {
                manageSecureStartUp.setEnabled(false);
                manageSecureStartUp.setSummary(R.string.disabled_because_no_backup_security);
            }
            if(manageSecureStartUp != null && !Utils.isVerizonSKU()){
                advancedCategory.removePreference(manageSecureStartUp);
            }
            //-- Asus: Verizon request
            //+++ suleman
            //advancedCategory.removePreference(root.findPreference(KEY_SCREEN_PINNING));
            //--- suleman
        }
        ...
        // BEGIN chilung_chen@asus.com, for CTA
        if (Utils.isCNSKU()) {
            //addPreferencesFromResource(R.xml.security_settings_cta);
        }
    ...
    private void maybeAddFingerprintPreference(PreferenceGroup securityCategory, int userId) {
        ...
        if (fingerprintPreference != null) {
            fingerprintPreference.setLayoutResource(R.layout.cnasusres_preference_parent);
    ...
    private void addTrustAgentSettings(PreferenceGroup securityCategory) {
        ...
        for (int i = 0; i < agents.size(); i++) {
            ...
            trustAgentPreference.setLayoutResource(R.layout.asusres_preference_material_parent);
    ...
    private void addSecureStartUp(PreferenceGroup securityCategory) {
        Preference manageSecureStartUp = new Preference(getContext());
        manageSecureStartUp.setKey("encryption_secure_start_up");
        manageSecureStartUp.setTitle(R.string.encryption_interstitial_header);
        manageSecureStartUp.setFragment("com.android.settings.CNSecureStartUpSettings");
        manageSecureStartUp.setLayoutResource(R.layout.asusres_preference_material_parent_nodivider);

        if (!mLockPatternUtils.isSecure(MY_USER_ID)) {
            manageSecureStartUp.setEnabled(false);
            manageSecureStartUp.setSummary(R.string.disabled_because_no_backup_security);
        } else {
            manageSecureStartUp.setEnabled(true);
//            int mRequestedPasswordQuality = getKeyQualityForCurrent();
            manageSecureStartUp.setSummary(R.string.encrypt_require_password);
        }
        securityCategory.addPreference(manageSecureStartUp);
    }
    ...
    private void warnAppInstallation() {
        ...
        mWarnInstallApps = new AlertDialog.Builder(getActivity()).setTitle(
                ...
                .setMessage(getResources().getString(R.string.asus_install_all_warning))
                .setPositiveButton(android.R.string.yes, this)
                .setNegativeButton(android.R.string.no, this)
                .show();

        Window dialogWindow = ((AlertDialog)mWarnInstallApps).getWindow();
    ...
    public boolean onPreferenceTreeClick(Preference preference) {
       ...
       } else if (KEY_SECURE_START_UP.equals(key)) {
            //++ Asus: Verizon requset
            ChooseLockSettingsHelper helper =
                    new ChooseLockSettingsHelper(this.getActivity(), this);
            boolean confirmationLaunched = helper.launchConfirmationActivityNeedPwd(
                    ENABLE_ENCRYPTION_REQUEST_VERIFY, preference.getTitle(), true);
            if(!confirmationLaunched){
                Log.w(TAG,"ENABLE_ENCRYPTION_REQUEST_VERIFY launch fail");
            }
            //-- Asus: Verizon requset
        } else if (KEY_CRYPT_KEEPER_ENCRYPT_TITLE.equals(key)) {
            Toast toast = Toast.makeText(getActivity(),R.string.security_toast, Toast.LENGTH_LONG);
    ...
    //++ Asus: Verizon request
    private int getKeyQualityForCurrent() {
    ...
    public void onActivityResult(int requestCode, int resultCode, Intent data) {
        ...
        if (requestCode == CHANGE_TRUST_AGENT_SETTINGS && resultCode == Activity.RESULT_OK) {
            ...
            boolean required = data.getBooleanExtra(EncryptionInterstitial.EXTRA_REQUIRE_PASSWORD,
                EncryptionInterstitial.DEFAULT_REQUIRE_PASSWORD);
            final LockPatternUtils lockPatternUtils = mChooseLockSettingsHelper.utils();
            lockPatternUtils.setCredentialRequiredToDecrypt(required);
        ...
        }else if(requestCode == ENABLE_ENCRYPTION_REQUEST_VERIFY && resultCode == Activity.RESULT_OK){
            mPwdType = data.getIntExtra("pwdtype",-1);
            mPwd = data.getStringExtra("pwd");
            final Context context = getActivity();
            final boolean accEn = AccessibilityManager.getInstance(context).isEnabled();
            final boolean required = mLockPatternUtils.isCredentialRequiredToDecrypt(/*!accEn*/ EncryptionInterstitial.DEFAULT_REQUIRE_PASSWORD);
            Intent intent = getEncryptionInterstitialIntent(context, getKeyQualityForCurrent(), required);
...
/*            if (!um.isAdminUser() || !tm.hasIccCard()) {
    ...
    public static class SecuritySubSettings extends SettingsPreferenceFragment
        ...
        private void createPreferenceHierarchy() {
            ...
            // CN Sku needs to remove the owner info
            if (mOwnerInfoPref != null && AsusLSUtils.mIsCNLockScreen) {
                root = getPreferenceScreen();
                if (root != null) {
                    root.removePreference(mOwnerInfoPref);
                }
                mOwnerInfoPref = null;
            }
                        ...
                        mOwnerInfoPref.setOnPreferenceClickListener(
                                new OnPreferenceClickListener() {
                                    ...
                                    public boolean onPreferenceClick(Preference preference) {
                                        ...
                                        startFragment(SecuritySubSettings.this,
                                                com.asus.settings.lockscreen.ui.AsusOwnerInfoSettings.class.getName(),
                                                R.string.owner_info_settings_title,
                                                0, null);
                                        // --- Asus: Porting N ---
                                        return true;
                                    }
        ...
        private void setupLockAfterPreference() {
            ...
            if (mDPM != null) {
                ...
                // Asus: Mark Code for Porting N
                /*
                final long maxTimeout = Math.max(0, adminTimeout - displayTimeout);
                mLockAfter.removeUnusableTimeouts(maxTimeout, admin);
                */
                // +++ Asus: Porting N - Use Android M logic +++
                if (adminTimeout > 0) {
                    Log.d(TAG, "setupLockAfterPreference adminTimeout:" + adminTimeout + ", displayTimeout:" + displayTimeout);
        ...
        private void updateLockAfterPreferenceSummary() {
            ...
            if (mLockAfter.isDisabledByAdmin()) {
                ...
                */
                // +++ Asus: Porting N - Copy Code Form M +++
                if (best == 0) {
                    //Immediately
                    mLockAfter.setSummary(getString(R.string.lock_after_timeout_summary, entries[best]));
                } else {
                    String timeout = formatElapsedTime(false, Long.valueOf(values[best].toString()));
                    mLockAfter.setSummary(getString(R.string.lock_after_timeout_summary, timeout));
                }

                //++ Update DB value
                try {
                    mLockAfter.setValue(String.valueOf(values[best]));
                    Settings.Secure.putInt(getContentResolver(),
                            Settings.Secure.LOCK_SCREEN_LOCK_AFTER_TIMEOUT, Integer.parseInt((String) values[best]));
                } catch (Exception e) {
        ...
        private void disableUnusableTimeouts(long maxTimeout) {
            ...
            //++ Asus: if device policy max timeout doesn't equal max item, then new a item for max timeout.
            boolean isChanged = false;
            if(values.length > 0) {
                long maxItemTimeout = Long.valueOf(values[values.length - 1].toString());
                if (maxTimeout <= maxItemTimeout
                        && maxTimeout != 0) {
                    String time_str = formatElapsedTime(true, maxTimeout);
                    revisedEntries.add(time_str);
                    revisedValues.add(String.valueOf(maxTimeout));
                    isChanged = true;
                }
            }
            //-- Asus: if device policy max timeout doesn't equal max item, then new a item for max timeout.

            if (revisedEntries.size() != entries.length || revisedValues.size() != values.length || isChanged) {
                ...
                if (userPreference <= maxTimeout) {
                    ...
                    //++ Asus: set max timeout as default value.
                    if (revisedValues.size() > 0 && userPreference != 0 &&
                            //Ken1_Yu,refresh UI when device policy change the maximumlock
        ...
        // +++ Asus: Porting N - Copy Code From M +++
        private static final int SECONDS_PER_MINUTE = 60;
        private static final int SECONDS_PER_HOUR = 60 * 60;
        private static final int SECONDS_PER_DAY = 24 * 60 * 60;

        public String formatElapsedTime(boolean hasMaxString, long millis) {
            ...
            boolean withSeconds = seconds != 0;
            if(withSeconds) {
                if (minutes > 0) {
                    if (minutes == 1) {
                        sb.append(context.getString(R.string.asus_lock_after_minute, minutes, seconds));
                    } else {
                        sb.append(context.getString(R.string.asus_lock_after_minutes, minutes, seconds));
                    }
                } else {
                    if (seconds == 1) {
                        sb.append(context.getString(R.string.asus_lock_after_second, seconds));
                    }else {
                        sb.append(context.getString(R.string.asus_lock_after_seconds, seconds));
                    }
                }
            } else {
                if (minutes == 1) {
                    sb.append(context.getString(R.string.asus_lock_after_minute_no_seconds, minutes));
                } else {
                    sb.append(context.getString(R.string.asus_lock_after_minutes_no_seconds, minutes));
                }
            }
            if (hasMaxString) {
                return context.getString(R.string.asus_lock_after_maximum, sb.toString());
            }
            return sb.toString();
        }
        // --- Asus: Porting N - Copy Code From M ---
