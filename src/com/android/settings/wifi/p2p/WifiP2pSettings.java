...
public class WifiP2pSettings extends SettingsPreferenceFragment
    ...
    // AMAX++: Show ip info for Wi-Fi Direct function
    private Preference mThisDeviceIp;
    // AMAX--: Show ip info for Wi-Fi Direct function
    ...
    private Preference mSearchingPref;
            ...
            if (WifiP2pManager.WIFI_P2P_STATE_CHANGED_ACTION.equals(action)) {
                ...
                if (networkInfo.isConnected()) {
                    ...
                    if (wifip2pinfo.isGroupOwner) {
                        mThisDeviceIp.setSummary("192.168.49.1");
                    } else {
                        String ipAddress = SystemProperties.get("dhcp.p2p.ipaddress", getActivity().getString(R.string.status_unavailable));
                        mThisDeviceIp.setSummary(ipAddress == null ? getActivity().getString(R.string.status_unavailable) : ipAddress);
                    }
                    // AMAX--: Show ip info for Wi-Fi Direct function
                } else if (mLastGroupFormed != true) {
                    // AMAX++: Show ip info for Wi-Fi Direct function
                    mThisDeviceIp.setSummary(getActivity().getString(R.string.status_unavailable));
                    // AMAX--: Show ip info for Wi-Fi Direct function
    ...
    public void onActivityCreated(Bundle savedInstanceState) {
                ...
                if (which == DialogInterface.BUTTON_POSITIVE) {
                    if (mWifiP2pManager != null) {
                        ...
                        mWifiP2pManager.setDeviceName(mChannel,
                                mDeviceNameText.getText().toString().trim(),
        ...
        PreferenceCategory deviceNameGroup = new PreferenceCategory(getPrefContext());
        deviceNameGroup.setLayoutResource(R.layout.asusres_preference_category_material_nodivider);
        preferenceScreen.addPreference(deviceNameGroup);

        mThisDevicePref = new Preference(getPrefContext());
        mThisDevicePref.setPersistent(false);
//        mThisDevicePref.setSelectable(false);
        mThisDevicePref.setLayoutResource(R.layout.cnasusres_preference_parent_nodivider);
        preferenceScreen.addPreference(mThisDevicePref);

        // AMAX++: Show ip info for Wi-Fi Direct function
        mThisDeviceIp = new Preference(getActivity());
        mThisDeviceIp.setTitle(R.string.wifi_advanced_ip_address_title);
//        preferenceScreen.addPreference(mThisDeviceIp);
        // AMAX--: Show ip info for Wi-Fi Direct function
    ...
    private void initButtonBar(){
		...
		if(mButtonBar != null) {
			...
			mButtonBar.addButton(1, R.drawable.asus_btn_refresh, getString(R.string.menu_stats_refresh));
...
//    }
  //tim++
    ...
    public boolean onPreferenceTreeClick(Preference preference) {
        ...
        } else if(preference == mThisDevicePref){
        	showDialog(DIALOG_RENAME);
        }
    ...
    public Dialog onCreateDialog(int id) {
        ...
        if (id == DIALOG_DISCONNECT) {
            ...
            contentTitle.setText(getActivity().getString(R.string.wifi_p2p_disconnect_title));
            contentMessage.setText(msg);
            ...
            contentTitle.setText(getActivity().getString(R.string.wifi_p2p_cancel_connect_title));
            contentMessage.setText(getActivity().getString(stringId, deviceName));
        	...
        	View view1 = LayoutInflater.from(activity).inflate(R.layout.cnasusres_alertdialog_textfield_with_title, null);
            TextView title = (TextView) view1.getRootView().findViewById(R.id.alertdialog_title);
            title.setText(activity.getResources().getString(R.string.wifi_p2p_menu_rename));
            
            mDeviceNameText = (EditText) view1.getRootView().findViewById(R.id.alertdialog_textfield);
            mDeviceNameText.setPadding(0, 0, 0, 0);
            mDeviceNameText.setFilters(new InputFilter[] {new Utf8ByteLengthFilter(32), new RemoveSpecialCharFilter(true)});
...
//                .setTitle(R.string.wifi_p2p_menu_rename)
        	...
        	int stringId = R.string.wifi_p2p_delete_group_content;
        	
            contentTitle.setText(getActivity().getString(R.string.wifi_p2p_delete_group_title));
            String groupName;
            if (mSelectedGroup == null){
            	groupName = (mSelectedGroupName == null) ? "" : mSelectedGroupName;
            } else {
            	groupName = mSelectedGroup.getGroupName();
            }
            contentMessage.setText(getActivity().getString(stringId, groupName));
    ...
    private void setDialogBOTTOM(AlertDialog dialog) {
    ...
    private void handlePeersChanged() {
        ...
        setPreferenceGroupChildrenLayout(mPeersGroup, R.layout.asusres_preference_material,
                R.layout.asusres_preference_material_nodivider, true);
    }

    @Override
    public void onPersistentGroupInfoAvailable(WifiP2pGroupList groups) {
        final CharSequence cs = "\\x";
        ...
        setPreferenceGroupChildrenLayout(mPersistentGroup, R.layout.asusres_preference_material,
                R.layout.asusres_preference_material_nodivider, true);
    }
...
//        mPersistentGroup.setEnabled(mWifiP2pEnabled);
    ...
    private void updateSearchMenu(boolean searching) {
       ...
       if(mWifiP2pSearching){
           if(mSearchingPref == null){
               mSearchingPref = new Preference(getPrefContext());
               mSearchingPref.setSummary(R.string.wifi_p2p_menu_searching);
               mSearchingPref.setOrder(0);
               mSearchingPref.setSelectable(false);
               mSearchingPref.setLayoutResource(R.layout.asusres_preference_material_nodivider);
           }
           mPeersGroup.removeAll();
           mPeersGroup.addPreference(mSearchingPref);
       } else{
           if(mSearchingPref != null)
               mPeersGroup.removePreference(mSearchingPref);
           if(mButtonBar != null && mButtonBar.getButton(1) != null){
               View icon = mButtonBar.getButton(1).findViewById(com.asus.cncommonres.R.id.icon);
               if(icon != null)
                   icon.clearAnimation();
           }
       }
