...
public class WifiConfigController implements TextWatcher,
    ...
    public WifiConfigController(WifiConfigUiBase parent, View view, AccessPoint accessPoint,
        ...
        if (mAccessPoint == null) { // new network
            ...
            if (mAccessPoint.isSaved()) {
                ...
                if(mMode != WifiConfigUiBase.MODE_MODIFY && !qrcode_string.equals("")){
                    LinearLayout.LayoutParams param = new LinearLayout.LayoutParams(
                             LinearLayout.LayoutParams.MATCH_PARENT,
                             LinearLayout.LayoutParams.MATCH_PARENT, 1.0f);
                    //((LinearLayout)(mView.findViewById(R.id.wifi_info))).setVisibility(View.VISIBLE);
                    ((LinearLayout)(mView.findViewById(R.id.wifi_qrcode))).setVisibility(View.VISIBLE);
                    //((LinearLayout)(mView.findViewById(R.id.wifi_info))).setLayoutParams(param);
                    ((LinearLayout)(mView.findViewById(R.id.wifi_qrcode))).setLayoutParams(param);

                    ImageView mQRcode = (ImageView) mView.findViewById(R.id.qrcode);
                    mQRcode.setImageBitmap(WifiQRCodeUtils.getQRCode(qrcode_string));
                }
            }
            ...
            if ((!mAccessPoint.isSaved() && !mAccessPoint.isActive())
                ...
                final CheckBox advancedTogglebox =
                        (CheckBox) mView.findViewById(R.id.wifi_advanced_togglebox);
                mView.findViewById(R.id.wifi_advanced_toggle).setVisibility(View.VISIBLE);
                // [MC1][Connectivity][AsusSettings][SingTel]Add SingTel customization for 802.1x +++
                if (isSingtelWifiSsid()) {
    ...
    // [MC1][Connectivity][AsusSettings][SingTel]Add SingTel customization for 802.1x +++
    private boolean isSingtelWifiSsid() {
        return (mAccessPoint != null) && (mAccessPoint.getSsidStr() != null) && (mAccessPoint.getSsidStr().equalsIgnoreCase("Singtel WIFI"));
    }
    // [MC1][Connectivity][AsusSettings][SingTel]Add SingTel customization for 802.1x ---
}
