...
public class ChartSweepView extends View {
    ...
    private void invalidateLabel() {
        if (mLabelTemplate != null && mAxis != null) {
            ...
            if (!AsusTelephonyUtils.isVerizon()) {
                ...
                mLabelValue = mAxis.buildLabel(getResources(), mLabelTemplate, mValue, mDisplayUnit);
            }
            // --- Mark_Huang@20151117: Verizon VZ_REQ_UI_15715
    ...
    public void setValue(long value) {
        Log.d(TAG, "setValue: " + value);  // +++ AMAX @ 20170119 7.1.1 Porting
        mValue = value;
        invalidateLabel();
    }
        ...
        switch (event.getAction()) {
            ...
            case MotionEvent.ACTION_UP: {
                if (mTouchMode == MODE_LABEL) {
                    ...
                    setTranslationY(0);
                    requestLayout();
                    Log.d(TAG, "onTouchEvent(): MotionEvent.ACTION_UP. mValue = " + mValue);  // +++ AMAX @ 20170119 7.1.1 Porting
                }
    ...
    public void updateValueFromPosition() {
        ...
        if (mFollowAxis == VERTICAL) {
            ...
            Log.d(TAG, "updateValueFromPosition(): mFollowAxis=" + mFollowAxis + ", setValue=" + mAxis.convertToValue(effectiveY));  // +++ AMAX @ 20170119 7.1.1 Porting
        } else {
            final float effectiveX = getX() - mMargins.left - parentContent.left;
            setValue(mAxis.convertToValue(effectiveX));
            Log.d(TAG, "updateValueFromPosition(): mFollowAxis=" + mFollowAxis + ", setValue=" + mAxis.convertToValue(effectiveX));  // +++ AMAX @ 20170119 7.1.1 Porting
        }
    }
    ...
    // +++ Mark_Huang@20151117: Verizon VZ_REQ_UI_15715
    public void setDisplayUnitBytes(SizeUnit displayUnit) {
