...
import com.android.settings.AsusTelephonyUtils.SizeUnit;
...
public class ChartDataUsageView extends ChartView {
    ...
    public void bindNetworkPolicy(NetworkPolicy policy) {
        ...
        if (policy.limitBytes != NetworkPolicy.LIMIT_DISABLED) {
            Log.d(TAG, "bindNetworkPolicy(): NetworkPolicy not LIMIT_DISABLED");  // +++ AMAX @ 20170119 7.1.1 Porting
        ...
        if (policy.warningBytes != NetworkPolicy.WARNING_DISABLED) {
            Log.d(TAG, "bindNetworkPolicy(): NetworkPolicy not WARNING_DISABLED");  // +++ AMAX @ 20170119 7.1.1 Porting
    ...
    public static class TimeAxis implements ChartAxis {
        ...
        @Override
        public long buildLabel(Resources res, SpannableStringBuilder builder, long value, SizeUnit displayUnit) {
            return buildLabel(res, builder, value);
        }
        // --- Mark_Huang@20151117: Verizon VZ_REQ_UI_15715
    ...
    public static class DataAxis implements ChartAxis {
        ...
        public long buildLabel(Resources res, SpannableStringBuilder builder, long value, SizeUnit displayUnit) {
            if (displayUnit == null) {
                Log.d(TAG, "buildLabel(): displayUnit is null");
                return buildLabel(res, builder, value);
            }
            value = MathUtils.constrain(value, 0, TrafficStats.TB_IN_BYTES);
            final BytesResult result = formatBytesInAssignedUnit(res, value,
                    Formatter.FLAG_SHORTER | Formatter.FLAG_CALCULATE_ROUNDED, displayUnit);
            if (result != null) {
                setText(builder, sSpanSize, result.value, "^1");
                setText(builder, sSpanUnit, result.units, "^2");
                return result.roundedBytes;
            } else {
                Log.d(TAG, "buildLabel(): Failed to set label since result is null!");
                return -1L;
            }
        }

        private BytesResult formatBytesInAssignedUnit(Resources res, long sizeBytes, int flags, SizeUnit displayUnit) {
            if (displayUnit == null) {
                Log.d(TAG, "formatBytesInAssignedUnit(): displayUnit is null");
                return Formatter.formatBytes(res, sizeBytes,
                            Formatter.FLAG_SHORTER | Formatter.FLAG_CALCULATE_ROUNDED);
            }
            float result = sizeBytes;
            long mult = 1;

            mult = displayUnit.getUnitbytes();
            result = result / mult;
            ...
            if (result < 1) {
                ...
                if ((flags & Formatter.FLAG_SHORTER) != 0) {
                    roundFactor = 10;
                    roundFormat = "%.1f";
                } else {
                    roundFactor = 100;
            ...
            final long roundedBytes =
                    (flags & Formatter.FLAG_CALCULATE_ROUNDED) == 0 ? 0
                            : (((long) Math.round(result * roundFactor)) * mult / roundFactor);

            final String units = displayUnit.toString();
    ...
    // +++ Mark_Huang@20151117: Verizon VZ_REQ_UI_15715
    public void setSweepViewDisplayUnit(SizeUnit displayUnit) {
        Log.d(TAG, "setSweepViewDisplayUnit(): displayUnit=" + displayUnit);
        if (mSweepLimit != null) {
            mSweepLimit.setDisplayUnitBytes(displayUnit);
        }
        if (mSweepWarning != null) {
            mSweepWarning.setDisplayUnitBytes(displayUnit);
        }
    }
    // --- Mark_Huang@20151117: Verizon VZ_REQ_UI_15715
}
