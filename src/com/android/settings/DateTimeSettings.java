...
import com.asus.cncommonres.AsusDatePicker;
...
public class DateTimeSettings extends SettingsPreferenceFragment
    ...
    private CNRestrictedSwitchPreference mAutoTimePref;
    ...
    private static final String TAG_DATE_PICKER = "DatetimeSettings_DatePicker";
    private static final String TAG_TIME_PICKER = "DatetimeSettings_TimePicker";
    ...
    private void initUI() {
        ...
        mAutoTimePref = (CNRestrictedSwitchPreference) findPreference(KEY_AUTO_TIME);
    ...
    public void onResume() {
        ...
        DatePickerDialog dpd = (DatePickerDialog) getFragmentManager().findFragmentByTag(TAG_DATE_PICKER);
        if (dpd != null) {
            //The dialog is already open so we need to set the listener again.
            dpd.setOnDateSetListener(this);
        }
        TimePickerDialog tpd = (TimePickerDialog) getFragmentManager().findFragmentByTag(TAG_TIME_PICKER);
        if (tpd != null) {
            //The dialog is already open so we need to set the listener again.
    ...
    //Ken1_yu, apply AMAX1.0 Datepicker
    private void showDatePicker() {
        final Calendar calendar = Calendar.getInstance();

        AsusDatePicker asusDatePicker = new AsusDatePicker(getContext(), new AsusDatePicker.ResultHandler() {
            @Override
            public void handle(Date date) {
                calendar.setTime(date);
                final Activity activity = getActivity();
                if (activity != null) {
                    setDate(activity, calendar.get(Calendar.YEAR), calendar.get(Calendar.MONTH), calendar.get(Calendar.DAY_OF_MONTH));
                    updateTimeAndDateDisplay(activity);
                }
            }
...
//                getActivity().getFragmentManager(), TAG_DATE_PICKER);
    }
    //Ken1_yu, apple AMAX1.0 Timepicker
    private void showTimePicker() {
        ...
        AsusTimePicker asusTimePicker = new AsusTimePicker(getContext(), new AsusTimePicker.ResultHandler() {
            ...
            public void handle(Date date) {
                ...
                final Activity activity = getActivity();
                if (activity != null) {
                    setTime(activity, calendar.get(Calendar.HOUR_OF_DAY), calendar.get(Calendar.MINUTE));
                    updateTimeAndDateDisplay(activity);
                }
            }
        },new Date(System.currentTimeMillis()));
...
//        is24Hour()).show(getFragmentManager(), TAG_TIME_PICKER);
    }
    @Override
