...
public class BatteryInfo {
    ...
    public void bindHistory(final UsageView view, BatteryDataParser... parsers) {
        ...
        long startTime = currentTime - timePeriod;
        String startTimeSubString = getTime(startTime);


        String endTimeSubString = "";
        String middleTimeSubString = "";
        if (remainingTimeUs != 0) {
            long endTime = currentTime + remainingTimeUs / 1000;
            endTimeSubString = getTime(endTime);
            long middleTime = (startTime + endTime)/2;
            middleTimeSubString = getTime(middleTime);
        }else{
            endTimeSubString = getTime(currentTime);
        }
...
//        }
        view.setBottomLabels(new CharSequence[]{startTimeSubString, middleTimeSubString, endTimeSubString});
    }
    ...
    private String getTime(Long time){
        ...
        endTimeSubString = timeString.substring(11,16);
        Log.d("Jack",timeString);
        return endTimeSubString;
    }
    ...
    public static BatteryInfo getBatteryInfo(Context context, Intent batteryBroadcast,
        ...
        if (!info.mCharging) {
            ...
            if (chargeTime > 0 && status != BatteryManager.BATTERY_STATUS_FULL && status != BatteryManager.BATTERY_STATUS_UNKNOWN) {
                ...
                if (plugType == BatteryManager.BATTERY_PLUGGED_AC) {
                    ...
                    int ac_charge_type = batteryBroadcast.getIntExtra(BatteryManager.EXTRA_STATUS, 0);
                    if(ac_charge_type == BatteryManager.BATTERY_STATUS_QUICK_CHARGING){
                        resId = R.string.power_charging_duration_ac_quick_charging;
                    }
                    //--
