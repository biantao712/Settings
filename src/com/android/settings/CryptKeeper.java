...
public class CryptKeeper extends Activity implements TextView.OnEditorActionListener,
    ...
    /** Message sent to us to check input. */
    private static final int MESSAGE_CHECK_INPUT = 3;
    ...
    private static final int COOL_DOWN_ATTEMPTS_VZW = 5;

    private static final String SAVE_FILE = "/dmagent/secure_start_config.dat";
    private static final String CONFIG_TAG = "config";
    private static final String FAILED_ATTEMPTS_KEY = "failed-attemtps";
    ...
    // how long we wait to check input again
    private static final int CHECK_INPUT_DELAY = 1000;

    private View mContentView;
    private boolean mInputDisplayed;
    ...
    private class DecryptTask extends AsyncTask<String, Void, Integer> {
        ...
        protected void onPostExecute(Integer failedAttempts) {
            ...
            } else if ((!mIsVZW && failedAttempts >= MAX_FAILED_ATTEMPTS)
                    || (mIsVZW && failedAttempts >= MAX_FAILED_ATTEMPTS_VZW)) {
                if(mIsVZW){
                    saveFailedAttempts(0);
                    doMasterClearAndWipeSDCard();
                }else{
                    doMasterClear();
                }
    ...
    private void doMasterClearAndWipeSDCard(){
        new AsyncTask<Void, Void, Void>(){
            @Override
            protected Void doInBackground(Void... params) {
                Log.w(TAG,"doMasterClearAndWipeSDCard");
                try{
                    IMountService mountService = getMountService();
                    Log.d(TAG,"mountservice:"+mountService);
                    if(mountService != null){
                        VolumeInfo[] volumes = mountService.getVolumes(0);
                        for(VolumeInfo vol : volumes) {
                            if (vol!=null && (vol.getType() == VolumeInfo.TYPE_PUBLIC) && vol.disk.isSd()){
                                String id = vol.getId();
                                Log.d(TAG,"format SDcard disk :"+vol.toString());
                                mountService.format(id);
                            }
                        }
                    }
                }catch(Exception exx){
                    Log.w(TAG,"doMasterClearAndWipeSDCard",exx);
                }
                return null;
            }
    ...
    private void saveFailedAttempts(int failedAttempts){
        try {
            FileOutputStream outStream = new FileOutputStream(SAVE_FILE,false);
            XmlSerializer out = Xml.newSerializer();
            out.setOutput(outStream, StandardCharsets.UTF_8.name());
            out.startDocument(StandardCharsets.UTF_8.name(), true);
            out.startTag(null, CONFIG_TAG)
            .startTag(null, FAILED_ATTEMPTS_KEY)
            .text(String.valueOf(failedAttempts))
            .endTag(null, FAILED_ATTEMPTS_KEY)
            .endTag(null, CONFIG_TAG)
            .endDocument();
            out.flush();
            outStream.flush();
            outStream.close();
            Log.d(TAG,"saved failedAttempts:"+failedAttempts);
    ...
    private int getFailedAttempts(){
        ...
        if(!file.exists()){
            Log.d(TAG,"secure start config file not exists,failed attempts 0");
            return failed;
        }
        ...
        try{
            ...
            while (eventType != XmlPullParser.END_DOCUMENT) {
                if(eventType == XmlPullParser.START_TAG
                        && FAILED_ATTEMPTS_KEY.equals(parser.getName())) {
                    String val = parser.nextText();
                    failed = Integer.parseInt(val, 10);
                    break;
                }
            ...
            Log.w(TAG,"XmlPullParserException",e);
        } catch (IOException e) {
            Log.w(TAG,"IOException",e);
        }finally{
            if(inStream != null)
    ...
    private void showAlmostWipeDialog(int remainAttempts) {
        cleanDialog();
        final AlertDialog dialog = new AlertDialog.Builder(CryptKeeper.this)
            .setMessage(TextUtils.expandTemplate(getString(R.string.crypt_keeper_warn_wipe),String.valueOf(remainAttempts)))
            .setNeutralButton(com.android.internal.R.string.ok, null)
            .create();
        dialog.show();
        mDialog = dialog;
    }
    private void handleBadAttemptForVZW(Integer failedAttempts){
        if (mLockPatternView != null) {
            mLockPatternView.setDisplayMode(DisplayMode.Wrong);
            mLockPatternView.setEnabled(true);
        }
        int remainAttempts = MAX_FAILED_ATTEMPTS_VZW - failedAttempts;
        final TextView status = (TextView) findViewById(R.id.status);
        int strId = remainAttempts > 1 ? R.string.asus_remain_password_attempts: R.string.asus_remain_password_attempt;
        status.setText(getResources().getString(strId, remainAttempts));
        ...
        if(remainAttempts == COOL_DOWN_ATTEMPTS_VZW || remainAttempts == 1){
            showAlmostWipeDialog(remainAttempts);
        }
    }
    private void handleBadAttempt(Integer failedAttempts) {
        ...
        if(mIsVZW){
            handleBadAttemptForVZW(failedAttempts);
            return;
        }
        ...
        if (!"".equals(progress) || isDebugView(FORCE_VIEW_PROGRESS)) {
            ...
            new AsyncTask<Void, Void, Void>() {
                ...
                public void onPostExecute(java.lang.Void v) {
                    ...
                    if(mIsVZW){
                        int remainAttempts = MAX_FAILED_ATTEMPTS_VZW - getFailedAttempts();
                        int strId = remainAttempts > 1 ? R.string.asus_remain_password_attempts: R.string.asus_remain_password_attempt;
                        status.setText(getResources().getString(strId, remainAttempts));
                        if(remainAttempts == COOL_DOWN_ATTEMPTS_VZW || remainAttempts == 1){
                            showAlmostWipeDialog(remainAttempts);
                        }else if(remainAttempts <= 0){
                            saveFailedAttempts(0);
                            doMasterClearAndWipeSDCard();
                        }
                    }
    ...
    private void showFactoryReset(final boolean corrupt) {
        ...
        if (corrupt) {
            ...
            ((TextView) findViewById(R.id.status)).setText(R.string.asus_crypt_keeper_data_corrupt_summary);
        } else {
            ((TextView) findViewById(R.id.title)).setText(R.string.crypt_keeper_failed_title);
            ((TextView) findViewById(R.id.status)).setText(R.string.asus_crypt_keeper_failed_summary);
        }
    ...
    private void updateProgress() {
        ...
        CharSequence status = getText(R.string.asus_crypt_keeper_setup_description);
        int percent = 0;
        try {
        ...
        try {
            ...
            if (time >= 0) {
                ...
                status = getText(R.string.asus_crypt_keeper_setup_time_remaining);
            }
        } catch (Exception e) {
     ...
     private void passwordEntryInit() {
        ...
        if (mWakeLock == null) {
            ...
            if (pm != null) {
                ...
                mReleaseWakeLockCountdown = 12; // 96 * 5 secs per click + 120 secs before we show this = 600
            }
        }
        ...
        if (mLockPatternView == null && !mCooldown) {
            ...
            // Detect whether input was ever displayed.  Input restart will be cancelled if it is
            // already displayed.
            if (mPasswordEntry != null) {
                mContentView = mPasswordEntry.getRootView().findViewById(android.R.id.content);
                if (mContentView != null) {
                    mContentView.getViewTreeObserver().addOnGlobalLayoutListener(new ViewTreeObserver.OnGlobalLayoutListener() {
                        public void onGlobalLayout() {
                            int inputHeight = imm.getInputMethodWindowVisibleHeight();
                            Log.d(TAG, "onGlobalLayout: " + inputHeight);

                            if (inputHeight > 0) {
                                mInputDisplayed = true;
                            }
                        }
                    });
                } else {
                    Log.d(TAG, "mContentView == null");
                }
            }

            // Check input after a delay.  This is needed when IME is not yet ready when the system
            // already wants to focus on the password field (ie. during boot up).  Input will keep
            // restarting until it is displayed.
            mHandler.sendEmptyMessageDelayed(MESSAGE_CHECK_INPUT, CHECK_INPUT_DELAY);
        }
    ...
    private void checkInput() {
        Log.d(TAG, "checkInput: mInputDisplayed=" + mInputDisplayed +
                ", mPasswordEntry=" + mPasswordEntry +
                ", mContentView=" + mContentView);

        if (!mInputDisplayed) {
            if (mPasswordEntry != null && mContentView != null) {
                ...
                // check again to make sure the input is displayed
                mHandler.sendEmptyMessageDelayed(MESSAGE_CHECK_INPUT, CHECK_INPUT_DELAY);
            }
        }
    }
