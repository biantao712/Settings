...
public class IccLockSettings extends SettingsPreferenceFragment
    ...
    private void updatePreferences() {
        // +++ AMAX @ 20170119 7.1.1 Porting
        if (null != mPinToggle && null != mPinDialog) {
            if (mPhone != null) {
    ...
    private void setDialogValues() {
        Log.d(TAG, "setDialogValues(): mDialogState = " + mDialogState);  // +++ AMAX @ 20170119 7.1.1 Porting
        mPinDialog.setText(mPin);
        String message = "";
        // +++ AMAX @ 20170119 7.1.1 Porting
        int attempts = 0;
        if (mPhone != null) {
            int sim1AttemptsLeft = SystemProperties.getInt("gsm.sim.pin1_count",0);
            int sim2AttemptsLeft = SystemProperties.getInt("gsm.sim.pin1_count.1",0);
            Log.d(TAG, "setDialogValues(): Attempts left SIM1 = " + sim1AttemptsLeft + " SIM2 = " + sim2AttemptsLeft);
            attempts = mPhone.getPhoneId() == 0 ? sim1AttemptsLeft : sim2AttemptsLeft;
        }
        // --- AMAX @ 20170119 7.1.1 Porting
        switch (mDialogState) {
            case ICC_LOCK_MODE:
    ...
    public void onPinEntered(EditPinPreference preference, boolean positiveResult) {
        Log.d(TAG, "onPinEntered(): positiveResult = " + positiveResult + " mDialogState = " + mDialogState);  // +++ AMAX @ 20170119 7.1.1 Porting
        if (!positiveResult) {
            resetDialogState();
            return;
        }
    ...
    public boolean onPreferenceTreeClick(Preference preference) {
        Log.d(TAG, "onPreferenceTreeClick(): preference = " + preference);  // +++ AMAX @ 20170119 7.1.1 Porting
    ...
    private void resetDialogState() {
        Log.d(TAG, "resetDialogState()");  // +++ AMAX @ 20170119 7.1.1 Porting
        mError = null;
    ...
    private OnTabChangeListener mTabListener = new OnTabChangeListener() {
        ...
        public void onTabChanged(String tabId) {
            ...
            mPhone = (sir == null) ? null
                : PhoneFactory.getPhone(SubscriptionManager.getPhoneId(sir.getSubscriptionId()));
            Log.d(TAG, "onTabChanged(): slotId = " + slotId + " mPhone = " + mPhone);  // +++ AMAX @ 20170119 7.1.1 Porting

            // The User has changed tab; update the body.
            updatePreferences();
            // +++ Mark_Huang@20160718: Reset dialog state when changing to another tab. Fix TT-838600.
            resetDialogState();
            // --- Mark_Huang@20160718: Reset dialog state when changing to another tab. Fix TT-838600.
        }
    };
