...
// +++ AMAX @ 20170119 7.1.1 Porting
    // +++ ckenken (ChiaHsiang_Kuo) @ 20170111 Verizon requirement
    public void updateTitle(boolean isMobile) {
        if (com.android.settings.AsusTelephonyUtils.isVerizon()) {
            if (isMobile) {
                setTitle(getContext().getString(R.string.vzw_cellular_data_usage));
            }
            if (!getTitle().toString().contains(getContext().getString(R.string.vzw_detail))) {
                setTitle(getContext().getString(R.string.vzw_detail) + getTitle());
            }
        }
    }
    // --- ckenken (ChiaHsiang_Kuo) @ 20170111 Verizon requirement
