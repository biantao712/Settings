...
public class AppDataUsagePreference extends Preference {
    ...
    public void setSummaryFormat(Context context, AsusTelephonyUtils.SizeUnit displayUnit) {
        if (mItem.restricted && mItem.total <= 0) {
            setSummary(com.android.settings.R.string.data_usage_app_restricted);
        } else {
            setSummary((!AsusTelephonyUtils.isVerizon()) ? Formatter.formatFileSize(context, mItem.total) : DataUsageList.formatFileSizeForVerizon(context, mItem.total, displayUnit));
        }
    }
    // --- AMAX @ 20170119 7.1.1 Porting
