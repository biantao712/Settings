...
public class CellDataPreference extends CustomDialogPreference implements TemplatePreference {
    ...
    private void updateChecked() {
        // +++ ckenken (ChiaHsiang_Kuo) @ 20161031 TT-893193 update the check state by checking DB directly instead of query by API
        // setChecked(mTelephonyManager.getDataEnabled(mSubId));
        int mobileDataDB = Global.getInt(getContext().getContentResolver(), Global.MOBILE_DATA, -1);
        Log.d(TAG, "mobileDataDB = " + mobileDataDB);
        boolean isDataEnable = (mobileDataDB == -1 || mobileDataDB == 1) ? true : false;
        setChecked(isDataEnable);
        // --- ckenken (ChiaHsiang_Kuo) @ 20161031 TT-893193 update the check state by checking DB directly instead of query by API

        // +++ ckenken (ChiaHsiang_Kuo) @ 20161031 TT-897174 update switch state when airplane mode change
        boolean isAirplaneMode = Global.getInt(getContext().getContentResolver(), Global.AIRPLANE_MODE_ON, 0) != 0;
        setEnabled(!isAirplaneMode);
        // --- ckenken (ChiaHsiang_Kuo) @ 20161031 TT-897174 update switch state when airplane mode change
    }
    ...
    private final DataStateListener mListener = new DataStateListener() {
        ...
        public void onChange(boolean selfChange) {
            Log.d(TAG, "onChange() non-uri"); // +++ AMAX @ 20170119 7.1.1 Porting
            updateChecked();
        }
    ...
    public abstract static class DataStateListener extends ContentObserver {
        ...
        public void setListener(boolean listening, int subId, Context context) {
            if (listening) {
                ...
                // +++ ckenken (ChiaHsiang_Kuo) @ 20161012 TT-886756   AMAX: Listen to the same url as single sim device
...
//                }
                ...
                // +++ ckenken (ChiaHsiang_Kuo) @ 20161031 TT-897174 update switch state when airplane mode change
                uri = Global.getUriFor(Global.AIRPLANE_MODE_ON);
                context.getContentResolver().registerContentObserver(uri, false, this);
                // --- ckenken (ChiaHsiang_Kuo) @ 20161031 TT-897174 update switch state when airplane mode change
            } else {
