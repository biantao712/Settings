...
public class ChartDataUsagePreference extends Preference {
    ...
    public void onBindViewHolder(PreferenceViewHolder holder) {
        ...
        chart.setBottomLabels(new CharSequence[] {
                ...
                Utils.formatDateRange(getContext(), (mStart + mEnd)/2 , (mStart + mEnd)/2),
    ...
    private CharSequence getLabel(long bytes, int str, int mLimitColor) {
        ...
        if (!AsusTelephonyUtils.isVerizon()) {
            label = TextUtils.expandTemplate(getContext().getText(str),
                    result.value, result.units);
        } else {
            int suffix = 0;
            if (DataUsageList.mDisplayInGB) {
                suffix = com.android.internal.R.string.gigabyteShort;
            } else {
                suffix = com.android.internal.R.string.megabyteShort;
            }
            label = TextUtils.expandTemplate(getContext().getText(str), TransformBytesFormat(bytes), getContext().getString(suffix));
        }
        // +++ AMAX @ 20170119 7.1.1 Porting
        return new SpannableStringBuilder().append(label, new ForegroundColorSpan(mLimitColor), 0);
    }
    // +++ AMAX @ 20170119 7.1.1 Porting
    private String TransformBytesFormat(long bytes) {
        float bytesInFloat = bytes;
        float bytesAfterDivided = 0;
        String roundFormat = "";
        if (DataUsageList.mDisplayInGB) {
            bytesAfterDivided = bytesInFloat / TrafficStats.GB_IN_BYTES;
        } else {
            bytesAfterDivided = bytesInFloat / TrafficStats.MB_IN_BYTES;
        }

        if (bytesAfterDivided >= 100) {
            roundFormat = "%.0f";
        } else if (bytesAfterDivided < 1) {
            roundFormat = "%.2f";
        } else {
            roundFormat = "%.1f";
        }
        return String.format(roundFormat, bytesAfterDivided);
    }
    // --- AMAX @ 20170119 7.1.1 Porting
