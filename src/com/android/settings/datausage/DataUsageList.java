...
public class DataUsageList extends DataUsageBase {
    ...
    // +++ ckenken (ChiaHsiang_Kuo) 20161025 VZ_REQ_UI_37886
    private static final String KEY_OPERATOR_WARNING = "operator_warning";
    // --- ckenken (ChiaHsiang_Kuo) 20161025 VZ_REQ_UI_37886
    ...
    // +++ ckenken (ChiaHsiang_Kuo) 20161025 VZ_REQ_UI_37886
    private Preference mOperatorWarning;
    // --- ckenken (ChiaHsiang_Kuo) 20161025 VZ_REQ_UI_37886

    // +++ Mark_Huang@20151117: Verizon VZ_REQ_UI_15715
    private MenuItem mMenuDisplayUnit;
    public static boolean mDisplayInGB = true;
    private static final String PREF_DISPLAY_IN_GB = "display_in_gb";
    // --- Mark_Huang@20151117: Verizon VZ_REQ_UI_15715
    ...
    public void onViewCreated(View v, Bundle savedInstanceState) {
        ...
        // +++ ckenken (ChiaHsiang_Kuo) 20161025 VZ_REQ_UI_37886
        mOperatorWarning = findPreference(KEY_OPERATOR_WARNING);
        // +++ Mark_Huang@20151109: Verizon VZ_REQ_UI_37886
        if (AsusTelephonyUtils.isVerizon()) {
            TelephonyManager telephonyManager = (TelephonyManager) getActivity().getSystemService(Context.TELEPHONY_SERVICE);
            boolean isVoiceCapable = telephonyManager.isVoiceCapable();
            Log.d(TAG, "onCreate(): isVoiceCapable = " + isVoiceCapable);
            if (mOperatorWarning != null) {
                mOperatorWarning.setSummary(getString(R.string.vzw_data_usage_disclaimer_2016_Nov, (isVoiceCapable)? getString(R.string.vzw_phone) : getString(R.string.vzw_tablet)));
            }
        }
        // --- Mark_Huang@20151109: Verizon VZ_REQ_UI_37886
        // --- ckenken (ChiaHsiang_Kuo) 20161025 VZ_REQ_UI_37886
    ...
    public void onCreateOptionsMenu(Menu menu, MenuInflater inflater) {
        ...
        // +++ Mark_Huang@20151117: Verizon VZ_REQ_UI_15715
        if (!AsusTelephonyUtils.isVerizon()) {
            menu.removeItem(R.id.vzw_data_usage_menu_display_unit);
            Log.d(TAG, "onCreateOptionsMenu(): Remove display unit menu item");
        } else {
            mMenuDisplayUnit = menu.findItem(R.id.vzw_data_usage_menu_display_unit);
            mMenuDisplayUnit.setOnMenuItemClickListener(new MenuItem.OnMenuItemClickListener() {
                @Override
                public boolean onMenuItemClick(MenuItem item) {
                    switch (item.getItemId()) {
                        case R.id.vzw_data_usage_menu_display_unit:
                            if (AsusTelephonyUtils.isVerizon()) {
                                mDisplayInGB = !mDisplayInGB;
                                Log.d(TAG, "onOptionsItemClick(): vzw_data_usage_menu_display_unit. mDisplayInGB = " + mDisplayInGB);
                                updateBody();
                                updateMenuTitle();
                                updateDetailData();
                                return true;
                            } else {
                                Log.d(TAG, "onOptionsItemClick(): Do nothing because of incorrect version");
                                return false;
                            }
                         default:
                             break;
    ...
    private void updateBody() {
        ...
        // +++ ckenken (ChiaHsiang_Kuo) @ 20161021 detail mode is moved to AppDataUsage.java, no longer need to init menu
    ...
    private void updateDetailData() {
        ...
        // +++ AMAX @ 20170119 7.1.1 Porting
        final String totalPhrase = (!AsusTelephonyUtils.isVerizon())? Formatter.formatFileSize(context, totalBytes) :
                formatFileSizeForVerizon(context, totalBytes, (mDisplayInGB)? AsusTelephonyUtils.SizeUnit.GB : AsusTelephonyUtils.SizeUnit.MB);
        // --- AMAX @ 20170119 7.1.1 Porting
    ...
    public void bindStats(NetworkStats stats, int[] restrictedUids) {
            ...
            if (UserHandle.isApp(uid)) {
                if (profiles.contains(new UserHandle(userId))) {
                    if (userId != currentUserId
                            && !(mEnableTwinApps && userId == mTwinAppsId)) { //[TwinApps]
        ...
        for (int i = 0; i < items.size(); i++) {
            ...
            // +++ AMAX @ 20170119 7.1.1 Porting
            preference.setSummaryFormat(getActivity(), mDisplayInGB ? AsusTelephonyUtils.SizeUnit.GB: AsusTelephonyUtils.SizeUnit.MB);
            // --- AMAX @ 20170119 7.1.1 Porting
...
// +++ AMAX @ 20170119 7.1.1 Porting
    // +++ Mark_Huang@20151117: Verizon VZ_REQ_UI_15715
    public static String formatFileSizeForVerizon(Context context, long sizeBytes, AsusTelephonyUtils.SizeUnit displayUnit) {
        if ((context == null) || (displayUnit == null)) {
            Log.d(TAG, "formatFileSizeForVerizon(): Failed to format file size since context or displayUnit is null");
            return "";
        }
        double sizeBytesLong = (double) sizeBytes;
        sizeBytesLong = sizeBytesLong / displayUnit.getUnitbytes();

        if (sizeBytesLong < 0.01D) {
            return "< 0.01" + displayUnit.toString();
        } else {
            return Math.round(sizeBytesLong * 100.0) / 100.0 + displayUnit.toString(); // Round result to 2nd decimal place
        }
    }
    // --- Mark_Huang@20151117: Verizon VZ_REQ_UI_15715
    ...
    public void updateMenuTitle() {
        ...
        if (AsusTelephonyUtils.isVerizon()) {
            if (mDisplayInGB) {
                if (mMenuDisplayUnit != null) {
                    Log.d(TAG, "updateMenuTitles(): set display-in-MB title");
                    mMenuDisplayUnit.setTitle(R.string.vzw_data_usage_menu_item_display_in_mb);
                }
            } else {
                if (mMenuDisplayUnit != null) {
                    Log.d(TAG, "updateMenuTitles(): set display-in-GB title");
                    mMenuDisplayUnit.setTitle(R.string.vzw_data_usage_menu_item_display_in_gb);
                }
            }
        }
        // --- Mark_Huang@20151117: Verizon VZ_REQ_UI_15715
