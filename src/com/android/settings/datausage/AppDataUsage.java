...
public class AppDataUsage extends DataUsageBase implements Preference.OnPreferenceChangeListener,
    ...
    private static final String MOBILE_MANAGER_PACKAGE_NAME = "com.asus.mobilemanager";
    private static final String MOBILE_MANAGER_ADVANCED_SETTINGS = ".soc.NetAppSettingsActivity";
    private static final int MOBILE_MANAGER_VERSION = 1540503000;
    ...
    public void onCreate(Bundle icicle) {
        ...
        if (mAppItem.key > 0) {
            if (mPackages.size() != 0) {
                ...
                try {
                    ...
                    if(TwinAppsUtil.isTwinAppsSupport(getContext())) {
                        int iTwinAppsId = UserManager.get(getContext()).getTwinAppsId();
                        if ((iTwinAppsId != -1) && (iTwinAppsId == UserHandle.getUserId(mAppItem.key))) {
                            mIcon = pm.getUserBadgedIcon(info.loadIcon(pm), new UserHandle(iTwinAppsId));
                        }
                    }
                    //[TwinApps] }
            ...
            if (!UserHandle.isApp(mAppItem.key)) {
                ...
                // BEGIN channing_yeh@asus.com
                // Add advance settings
                removePreference(KEY_ADVANCED_SETTINGS);
                // END channing_yeh@asus.com
            } else {
                ...
                //tim_hu@asus.com
                boolean isMobileManagerEnable = isMobileManagerEnable();
                mRestrictBackground.setVisible(!isMobileManagerEnable);
                mUnrestrictedData.setVisible(!isMobileManagerEnable);
                //---

                // Add advance settings
                if (isMobileManagerEnable()) {
                    mAdvancedSettings = findPreference(KEY_ADVANCED_SETTINGS);
                } else {
                    removePreference(KEY_ADVANCED_SETTINGS);
                }
                // END channing_yeh@asus.com
    ...
    public boolean onPreferenceTreeClick(Preference preference) {
        ...
        // BEGIN channing_yeh@asus.com
        // Add advance settings
        else if (isMobileManagerEnable() && preference == mAdvancedSettings) {
            Intent intent = new Intent();
            intent.setComponent(new ComponentName(MOBILE_MANAGER_PACKAGE_NAME,
                        MOBILE_MANAGER_PACKAGE_NAME + MOBILE_MANAGER_ADVANCED_SETTINGS));
            intent.putExtra("pkg", mPackageName);
            intent.putExtra("showAdvancedSettings", true);
    ...
    private void bindData() {
        ...
        mTotalUsage.setSummary((!AsusTelephonyUtils.isVerizon()) ? Formatter.formatFileSize(context, totalBytes) : DataUsageList.formatFileSizeForVerizon(context, totalBytes, (DataUsageList.mDisplayInGB) ? AsusTelephonyUtils.SizeUnit.GB : AsusTelephonyUtils.SizeUnit.MB ));
        mForegroundUsage.setSummary((!AsusTelephonyUtils.isVerizon()) ? Formatter.formatFileSize(context, foregroundBytes) : DataUsageList.formatFileSizeForVerizon(context, foregroundBytes, (DataUsageList.mDisplayInGB) ? AsusTelephonyUtils.SizeUnit.GB : AsusTelephonyUtils.SizeUnit.MB ));
        mBackgroundUsage.setSummary((!AsusTelephonyUtils.isVerizon()) ? Formatter.formatFileSize(context, backgroundBytes) : DataUsageList.formatFileSizeForVerizon(context, backgroundBytes, (DataUsageList.mDisplayInGB) ? AsusTelephonyUtils.SizeUnit.GB : AsusTelephonyUtils.SizeUnit.MB ));
        // --- AMAX @ 20170119 7.1.1 Porting
    }
    ...
    // BEGIN channing_yeh@asus.com
    // Add advance settings
    private boolean isMobileManagerEnable() {
        PackageManager pm = getPackageManager();
        int version = 0;
        try {
            PackageInfo packageInfo = pm.getPackageInfo(MOBILE_MANAGER_PACKAGE_NAME, 0);
            if (packageInfo.applicationInfo.enabled) {
                version = packageInfo.versionCode;
            }
        } catch (PackageManager.NameNotFoundException e) {
        }
        return version >= MOBILE_MANAGER_VERSION;
    }
    // END channing_yeh@asus.com

    //tim_hu@asus.com
    private boolean isMobileDataEnable(int uid){
        Map<Integer, List<String>> uidInterfaceRules = com.android.settings.Utils.getUidInterfaceRules();
        List<String> rulesList = uidInterfaceRules.get(uid);
        if(rulesList != null){
            return !rulesList.contains(getModuleName());
        }
