...
public class UsageGraph extends View {
    ...
    private final Paint mFillDottedPaint;
    ...
    private int mUsageGraphLineWidth = 0;
    ...
    public UsageGraph(Context context, @Nullable AttributeSet attrs) {
        ...
        mUsageGraphLineWidth = resources.getDimensionPixelSize(R.dimen.usage_graph_line_width);
        mLinePaint.setStrokeWidth(mUsageGraphLineWidth);

        mFillPaint = new Paint(mLinePaint);
        mFillPaint.setStyle(Style.FILL);
        mFillPaint.setPathEffect(new CornerPathEffect(1));

        mDividerPaint = new Paint(mLinePaint);
        mDividerPaint.setColor(context.getColor(R.color.usage_graph_divier_color));
        ...
        mDottedPaint.setPathEffect(new CornerPathEffect(mCornerRadius));
        mDottedPaint.setColor(context.getColor(R.color.usage_graph_dots));

        mFillDottedPaint = new Paint(mDottedPaint);
        mFillDottedPaint.setStyle(Style.FILL);
        mFillDottedPaint.setPathEffect(new CornerPathEffect(1));
    ...
    private void updateGradient() {
        mFillPaint.setShader(new LinearGradient(0, 0, 0, getHeight(),
                Color.WHITE, Color.WHITE, TileMode.CLAMP));
    }
...
//                mMiddleDividerTint);
    ...
    private void drawFillProjection(Canvas canvas) {
        mPath.reset();
        int x = mLocalPaths.keyAt(mLocalPaths.size() - 2);
        int y = mLocalPaths.valueAt(mLocalPaths.size() - 2);
        mPath.moveTo(x, y);
        mPath.lineTo(canvas.getWidth() - mUsageGraphLineWidth * 10, mProjectUp ? 0 : canvas.getHeight() - mUsageGraphLineWidth);
        if(mProjectUp){
            mPath.lineTo(canvas.getWidth() - mUsageGraphLineWidth * 10 , canvas.getHeight() - mUsageGraphLineWidth);
        }
        mPath.lineTo(x, canvas.getHeight() - mUsageGraphLineWidth);
        canvas.drawPath(mPath, mFillDottedPaint);
    }

    private void drawLinePath(Canvas canvas) {
        mPath.reset();
        mPath.moveTo(mLocalPaths.keyAt(0) + mUsageGraphLineWidth, mLocalPaths.valueAt(0));
        for (int i = 1; i < mLocalPaths.size(); i++) {
            int x = mLocalPaths.keyAt(i);
            int y = mLocalPaths.valueAt(i);
            if (y == PATH_DELIM) {
                if (++i < mLocalPaths.size()) {
                    y = mLocalPaths.valueAt(i);
                    if(mLocalPaths.valueAt(i) == getHeight()){
                        y = mLocalPaths.valueAt(i) - mUsageGraphLineWidth;
                    }
                    mPath.moveTo(mLocalPaths.keyAt(i), y);
                }
            } else {
                if(y == getHeight()){
                    y = mLocalPaths.valueAt(i) - mUsageGraphLineWidth;
    ...
    private void drawFilledPath(Canvas canvas) {
        ...
        if(lastStartX == 0){
            lastStartX = mUsageGraphLineWidth;
        }
        ...
        for (int i = 1; i < mLocalPaths.size(); i++) {
            ...
            int y = mLocalPaths.valueAt(i);
            if (y == PATH_DELIM) {
                mPath.lineTo(mLocalPaths.keyAt(i - 1), getHeight() - mUsageGraphLineWidth);
                mPath.lineTo(lastStartX, getHeight() - mUsageGraphLineWidth);
                mPath.close();
                if (++i < mLocalPaths.size()) {
