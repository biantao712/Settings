...
public class UsbModeChooserActivity extends Activity {
    ...
    private ArrayList<Integer> mModeArray;

    private static final String PROPERTY_KEY_SURA_FUNCTION = "ro.asus.sura";
    private static final String SURA_ENABLE = "1";
    private static final String SURA_DISABLE = "0";
    ...
    protected void onCreate(@Nullable Bundle savedInstanceState) {
        ...
        for (int mode: mModeArray) {
            if (mBackend.isModeSupported(mode) && !mBackend.isModeDisallowedBySystem(mode)) {
                inflateOption(mode, current == mode, container, mBackend.isModeDisallowed(mode));
            }
        }
    }

    private void initModeArray() {
        mModeArray = new ArrayList<>();
        for (int defaultMode : DEFAULT_MODES) {
            mModeArray.add(defaultMode);
        }

        if (SystemProperties.get(PROPERTY_KEY_SURA_FUNCTION, SURA_DISABLE).equals(SURA_ENABLE)) {
            mModeArray.add(UsbBackend.MODE_POWER_SINK | UsbBackend.MODE_DATA_CDROM);
        }
    }
    ...
    private static int getSummary(int mode) {
        switch (mode) {
            ...
            case UsbBackend.MODE_POWER_SINK | UsbBackend.MODE_DATA_CDROM:
                return R.string.usb_use_CDROM_desc;
        }
        return 0;
    }
