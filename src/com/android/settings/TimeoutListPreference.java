...
public class TimeoutListPreference extends RestrictedListPreference {
    ...
    public void removeUnusableTimeouts(long maxTimeout, EnforcedAdmin admin) {
        ...
        for (int i = 0; i < mInitialValues.length; ++i) {
            ...
            if (timeout <= maxTimeout && timeout != 0L) {
        ...
        if (revisedEntries.size() != getEntries().length) {
            ...
            mAdmin = admin;
            if (userPreference <= maxTimeout && userPreference != 0) {
                setValue(String.valueOf(userPreference));
            } else if (revisedValues.size() > 0 && userPreference != 0 &&
                ...
                //++Ken1_Yu, if nothing match then set the maximum value in the entry
                // Comment out by Jim:
                // updateTimeoutPreferenceDescription() can only be used in DisplaySettings.java
                /*
                String revise_timout = (String) revisedValues.get(revisedValues.size() - 1);
                screenTimeoutPreference.setValue(revise_timout);
                int value = Integer.parseInt(revise_timout);
