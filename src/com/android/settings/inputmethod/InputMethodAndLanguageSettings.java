...
public class InputMethodAndLanguageSettings extends SettingsPreferenceFragment
    ...
    private static final String PACKAGE_PC_LINK = "com.asus.linkrim";

    //BEGIN: Jeffrey_Chiang@asus.com
    private static final String KEY_POINTER_SETTING_CATEGORY = "pointer_settings_category";
    private static final String KEY_TOUCHPAD_MODE_SELECTOR = "touchpad_mode_selector";
    private static final String KEY_RIGHT_BUTTON_MAPPING_SELECTOR = "right_button_mapping_selector";
    private static final String KEY_MIDDLE_BUTTON_MAPPING_SELECTOR = "middle_button_mapping_selector";
    ...
    private ListPreference mRightButtonMappingSelectorPref;
    private ListPreference mMiddleButtonMappingSelectorPref;
    //END: Jeffrey_Chiang@asus.com
    ...
    private boolean hideUserDictionary = false;
    ...
    public void onCreate(Bundle icicle) {
        ...
        // BEGIN adrian_sung@asus.com, to hide spell checker and user dictionary
        if (!mShowsOnlyFullImeAndKeyboardList) {
            Preference prefSpellChecker = findPreference("spellcheckers_settings");
            getPreferenceScreen().removePreference(prefSpellChecker);
            Preference prefUserDictionary = findPreference("key_user_dictionary_settings");
            getPreferenceScreen().removePreference(prefUserDictionary);
            hideUserDictionary = true;
        }
        // END adrian_sung@asus.com

        //BEGIN: Steven_Chao@asus.com
        // Touchpad pointer style setting
        mTouchpadModeSelectorPref = (ListPreference) findPreference(KEY_TOUCHPAD_MODE_SELECTOR);
        if (mTouchpadModeSelectorPref != null) {
            int touch_mode = getTouchpadModeSetting(0);
            mTouchpadModeSelectorPref.setValue(String.valueOf(touch_mode));
            mTouchpadModeSelectorPref.setSummary(mTouchpadModeSelectorPref.getEntries()[touch_mode]);
            mTouchpadModeSelectorPref.setOnPreferenceChangeListener(this);
        }

        // Touchpad/Mouse right button mapping
        mRightButtonMappingSelectorPref = (ListPreference) findPreference(KEY_RIGHT_BUTTON_MAPPING_SELECTOR);
        if (mRightButtonMappingSelectorPref != null) {
            int right_button_mapping = getRightButtonMappingSetting(MotionEvent.BUTTON_SECONDARY);
            mRightButtonMappingSelectorPref.setValue(String.valueOf(right_button_mapping));
            mRightButtonMappingSelectorPref.setSummary(mRightButtonMappingSelectorPref.getEntry());
            mRightButtonMappingSelectorPref.setOnPreferenceChangeListener(this);
        }

        // Touchpad/Mouse middle button mapping
        mMiddleButtonMappingSelectorPref = (ListPreference) findPreference(KEY_MIDDLE_BUTTON_MAPPING_SELECTOR);
        if (mMiddleButtonMappingSelectorPref != null) {
            int middle_button_mapping = getMiddleButtonMappingSetting(MotionEvent.BUTTON_TERTIARY);
            mMiddleButtonMappingSelectorPref.setValue(String.valueOf(middle_button_mapping));
            mMiddleButtonMappingSelectorPref.setSummary(mMiddleButtonMappingSelectorPref.getEntry());
            mMiddleButtonMappingSelectorPref.setOnPreferenceChangeListener(this);
        }
        PackageManager pm = getPackageManager();
        boolean hasKeyboardFeature =
                 pm.hasSystemFeature(PackageManager.FEATURE_ASUS_KEYBOARD);
        if(!hasKeyboardFeature) {
            PreferenceCategory touchpadSettingCategory = (PreferenceCategory) getPreferenceScreen().findPreference(KEY_POINTER_SETTING_CATEGORY);
            if (touchpadSettingCategory != null) {
                //touchpadSettingCategory.setTitle(R.string.mouse_setting_title);
                touchpadSettingCategory.removePreference(mTouchpadModeSelectorPref);
            }
        }
...
//                imm.showInputMethodPicker(false /* showAuxiliarySubtypes */);
    ...
    //++ jack_qi
    private void showAsusInputMethodPicker(){

        String currentInputMethodId = Settings.Secure.getString(getContentResolver(),
                Settings.Secure.DEFAULT_INPUT_METHOD);
        int currentInputMethodIndex = 0;
        int imisSize = mInputMethodPreferenceList.size();
        String[] imisName = new String[imisSize];
        for (int i = 0; i < imisSize; i ++){
            imisName[i] = mInputMethodPreferenceList.get(i).getTitle().toString();
            if(currentInputMethodId != null && currentInputMethodId.equals(mInputMethodPreferenceList.get(i).getInputMethodInfo().getId())){
                currentInputMethodIndex = i;
            }
        }

        String switchDefaultInputMethodTitle = getContext().getResources().getString(R.string.switch_default_inputmethod_dialog_title);
        AlertDialog.Builder alertDialog_single_list_item = new AlertDialog.Builder(getContext())
                .setSingleChoiceItems(imisName, currentInputMethodIndex, new DialogInterface.OnClickListener() {

                    @Override
                    public void onClick(DialogInterface dialog, int which) {
                        if( which <= mInputMethodPreferenceList.size()){
                            Settings.Secure.putString(getContentResolver(), Settings.Secure.DEFAULT_INPUT_METHOD,
                                    mInputMethodPreferenceList.get(which).getInputMethodInfo().getId());
                            dialog.dismiss();
                        }
                    }
                })
                .setTitle(switchDefaultInputMethodTitle)
                .setNegativeButton(android.R.string.cancel, null)
                .setNeutralButton(getContext().getResources().getString(R.string.keyboard_and_inputmethod_setting),
            ...
            public void onClick(DialogInterface dialog, int which) {
                ...
                startFragment(InputMethodAndLanguageSettings.this,
                        ...
                        R.string.keyboard_and_inputmethod_setting_title, -1, null);
            }
        });
        Dialog dialog = alertDialog_single_list_item.create();
    ...
    public boolean onPreferenceChange(Preference preference, Object value) {
        ...
        //BEGIN: Steven_Chao@asus.com
        else if (preference == mTouchpadModeSelectorPref) {
            if (value instanceof String) {
                putTouchpadModeSetting(Integer.parseInt((String)value));
                return true;
            }
        } else if (preference == mRightButtonMappingSelectorPref) {
            if (value instanceof String) {
                putRightButtonMappingSetting(Integer.parseInt((String)value));
                return true;
            }
        } else if (preference == mMiddleButtonMappingSelectorPref) {
            if (value instanceof String) {
                putMiddleButtonMappingSetting(Integer.parseInt((String)value));
                return true;
            }
        }
        //END: Steven_Chao@asus.com
    ...
    private void updateInputMethodPreferenceViews() {
        ...
        synchronized (mInputMethodPreferenceList) {
            ...
            for (int i = 0; i < N; ++i) {
                ...
                if (!imi.getPackageName().equals(PACKAGE_PC_LINK)){
            ...
            for (int i = 0; i < mInputMethodPreferenceList.size(); ++i) {
                final InputMethodPreference pref = mInputMethodPreferenceList.get(i);
                if(i == mInputMethodPreferenceList.size() - 1){
                    pref.setLayout(R.layout.cn_restricted_switch_preference_nodivider_with_arrow);
                }else{
                    pref.setLayout(R.layout.cn_restricted_switch_preference_with_arrow);
                }
    ...
    private void updateHardKeyboards() {
        ...
        if (false/*!mHardKeyboardPreferenceList.isEmpty()*/) {
    ...
    public static final Indexable.SearchIndexProvider SEARCH_INDEX_DATA_PROVIDER =
        ...
        public List<SearchIndexableRaw> getRawDataToIndex(Context context, boolean enabled) {
            ...
            indexable = new SearchIndexableRaw(context);
            indexable.key = KEY_RIGHT_BUTTON_MAPPING_SELECTOR;
            indexable.title = context.getString(R.string.right_button_mapping_selector);
            indexable.screenTitle = screenTitle;
            indexables.add(indexable);

            indexable = new SearchIndexableRaw(context);
            indexable.key = KEY_MIDDLE_BUTTON_MAPPING_SELECTOR;
            indexable.title = context.getString(R.string.middle_button_mapping_selector);
            indexable.screenTitle = screenTitle;
            indexables.add(indexable);
            // Game controllers.
            if (haveInputDeviceWithVibrator()) {
    ...
    private int getTouchpadModeSetting(int defaultValue) {
        ...
        try {
             mode = Settings.System.getInt(getContentResolver(),
                     Settings.System.TOUCHPAD_MODE);
        } catch (SettingNotFoundException snfe) {
        }
        return mode;
    }

    private void putTouchpadModeSetting(int mode) {
        Settings.System.putInt(getContentResolver(), Settings.System.TOUCHPAD_MODE, mode);
        mTouchpadModeSelectorPref.setSummary(mTouchpadModeSelectorPref.getEntries()[mode]);
    }

    private int getRightButtonMappingSetting(int defaultValue) {
        int btn = defaultValue;
        try {
            btn = Settings.System.getInt(getContentResolver(),
                    Settings.System.RIGHT_BUTTON_MAPPING);
        } catch (SettingNotFoundException snfe) {
        }
        return btn;
    }

    private void putRightButtonMappingSetting(int btn) {
        Settings.System.putInt(getContentResolver(), Settings.System.RIGHT_BUTTON_MAPPING, btn);
        mRightButtonMappingSelectorPref.setValue(String.valueOf(btn));
        mRightButtonMappingSelectorPref.setSummary(mRightButtonMappingSelectorPref.getEntry());
    }

    private int getMiddleButtonMappingSetting(int defaultValue) {
        int btn = defaultValue;
        try {
            btn = Settings.System.getInt(getContentResolver(),
                    Settings.System.MIDDLE_BUTTON_MAPPING);
        } catch (SettingNotFoundException snfe) {
        }
        return btn;
    }

    private void putMiddleButtonMappingSetting(int btn) {
        Settings.System.putInt(getContentResolver(), Settings.System.MIDDLE_BUTTON_MAPPING, btn);
        mMiddleButtonMappingSelectorPref.setValue(String.valueOf(btn));
        mMiddleButtonMappingSelectorPref.setSummary(mMiddleButtonMappingSelectorPref.getEntry());
    }
    //END: Jeffrey_Chiang@asus.com
}
