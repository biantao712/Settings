...
public class NotificationBackend {
    ...
    //[TwinApps] {
    /* @hide */
    public AppRow loadAppRowAsUser(Context context, PackageManager pm, PackageInfo app) {
        final AppRow row = loadAppRow(context, pm, app.applicationInfo);
        row.cantBlock = Utils.isSystemPackage(context.getResources(), pm, app);
        row.lockScreenSecure = new LockPatternUtils(context).isSecure(
                UserHandle.getUserId(app.applicationInfo.uid));
        final String[] nonBlockablePkgs = context.getResources().getStringArray(
                com.android.internal.R.array.config_nonBlockableNotificationPackages);
        if (nonBlockablePkgs != null) {
            int N = nonBlockablePkgs.length;
    ...
    public boolean setAllowed(String pkg, int uid, int importance) {
        try {
            //sINM.setAllowed(pkg, uid, importance);
            setImportance(pkg, uid, (importance == 0 ? Ranking.IMPORTANCE_NONE : Ranking.IMPORTANCE_UNSPECIFIED));
            return true;
        } catch (Exception e) {
            Log.w(TAG, "Error calling NoMan", e);
            return false;
    ...
    public int getAllowed(String pkg, int uid) {
        try {
            //return sINM.getAllowed(pkg, uid);
            return (getImportance(pkg, uid) != Ranking.IMPORTANCE_NONE ? 1 : 0);
        } catch (Exception e) {
            Log.w(TAG, "Error calling NoMan", e);
            return 0;
        }
    }
    public boolean setStyleImportance(String pkg, int uid, int importance) {
        try {
            sINM.setStyleImportance(pkg, uid, importance);
            return true;
        } catch (Exception e) {
            Log.w(TAG, "Error calling NoMan", e);
            return false;
    ...
    public boolean setNotifySettings(String pkg, int uid, int type, int value) {
        try {
            sINM.setNotifySettings(pkg, uid, type, value);
            return true;
        } catch (Exception e) {
            Log.w(TAG, "Error calling set notify settings:", e);
            return false;
        }
    }


    public int getStyleIcon(String pkg, int uid) {
        try {
            return sINM.getStyleIcon(pkg, uid);
        } catch (Exception e) {
            Log.w(TAG, "Error calling NoMan", e);
            return 0;
        }
    ...
    public boolean setStyleIcon(String pkg, int uid, int importance) {
        try {
            sINM.setStyleIcon(pkg, uid, importance);
            return true;
        } catch (Exception e) {
            Log.w(TAG, "Error calling NoMan", e);
            return false;
    ...
    public int getStyleHeadsUp(String pkg, int uid) {
        try {
            return sINM.getStyleHeadsUp(pkg, uid);
        } catch (Exception e) {
            Log.w(TAG, "Error calling NoMan", e);
            return 0;
        }
    }
    public boolean setStyleHeadsUp(String pkg, int uid, int importance) {
        try {
            sINM.setStyleHeadsUp(pkg, uid, importance);
            return true;
        } catch (Exception e) {
            Log.w(TAG, "Error calling NoMan", e);
            return false;
    ...
    public int getStyleLockScreen(String pkg, int uid) {
        try {
            return sINM.getStyleLockScreen(pkg, uid);
        } catch (Exception e) {
            Log.w(TAG, "Error calling NoMan", e);
            return 0;
        }
    }
    public boolean setStyleLockScreen(String pkg, int uid, int importance) {
        try {
            sINM.setStyleLockScreen(pkg, uid, importance);
            return true;
        } catch (Exception e) {
            Log.w(TAG, "Error calling NoMan", e);
            return false;
    ...
    public int getStyleImportance(String pkg, int uid) {
        try {
            return sINM.getStyleImportance(pkg, uid);
        } catch (Exception e) {
            Log.w(TAG, "Error calling NoMan", e);
            return 0;
        }
    ...
    public int getNotifySettings(String pkg, int uid,int type) {
        try {
            int value =sINM.getNotifySettings(pkg, uid, type);
            return value;
        } catch (Exception e) {
            Log.w(TAG, "Error calling notify settings", e);
            return 0;
        }
    }
