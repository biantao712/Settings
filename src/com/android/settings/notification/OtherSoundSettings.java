...
public class OtherSoundSettings extends SettingsPreferenceFragment implements Indexable {
    ...
    private static final SettingPref PREF_CHARGING_SOUNDS = new SettingPref(
        ...
        @Override
        public boolean isApplicable(Context context) {
            return false;//hide charging sounds
        }
    };
    ...
    private static final SettingPref PREF_VIBRATE_ON_TOUCH = new SettingPref(
        ...
        public boolean isApplicable(Context context) {
	    //Sharon+++show vibrate on touch UI or not
        //boolean mVibrationIntensityCapable = Boolean.valueOf(SystemProperties.get("ro.config.zf3"));
        boolean mVibrationIntensityCapable = context.getPackageManager().hasSystemFeature(PackageManager.FEATURE_ASUS_VIBRATION_INTENSITY);
        Log.d(TAG,"test+mVibrationIntensityCapable="+mVibrationIntensityCapable);
        if (!mVibrationIntensityCapable) {//not zf3
            Log.d(TAG,"onCreate+vibrate on touch+not zf3");
            return hasHaptic(context);
		}
		else {//zf3
		    Log.d(TAG,"onCreate+vibrate on touch+zf3");
		    return false;
		}
		//Sharon---show vibrate on touch UI or not
        }
