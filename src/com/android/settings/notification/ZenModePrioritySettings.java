...
import com.android.settings.Utils;//chunghung_lin@asus.com
...
public class ZenModePrioritySettings extends ZenModeSettingsBase implements Indexable {
    ...
    public void onCreate(Bundle savedInstanceState) {
        ...
        if (Utils.isVoiceCapable(mContext)) {// chunghung_lin@asus.com
            ...
            mCalls.setOnPreferenceChangeListener(new OnPreferenceChangeListener() {
                ...
                public boolean onPreferenceChange(Preference preference, Object newValue) {
                    ...
                    if (DEBUG)
                        Log.d(TAG, "onPrefChange allowCalls=" + allowCalls + " allowCallsFrom="
                                + ZenModeConfig.sourceToString(allowCallsFrom));
                    savePolicy(getNewPriorityCategories(allowCalls, Policy.PRIORITY_CATEGORY_CALLS), allowCallsFrom,
        ...
        if (Utils.isVoiceCapable(mContext)) {// chunghung_lin@asus.com
            mRepeatCallers.setSummary(mContext.getString(R.string.zen_mode_repeat_callers_summary,
                        mContext.getResources().getInteger(com.android.internal.R.integer.config_zen_repeat_callers_threshold)));
    ...
    private static void addSources(DropDownPreference pref) {
        pref.setEntries(new CharSequence[]{
                pref.getContext().getString(R.string.zen_mode_from_anyone3),
                pref.getContext().getString(R.string.zen_mode_from_contacts_only),
                pref.getContext().getString(R.string.zen_mode_from_favorites_only),
