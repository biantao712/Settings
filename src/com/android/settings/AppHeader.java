...
public class AppHeader {
    ...
    public static void createAppHeader(SettingsPreferenceFragment fragment, Drawable icon,
            CharSequence label, String pkgName, int uid, boolean titleInCenter) {
        createAppHeader(fragment, icon, label, null, pkgName, uid, 0, null, titleInCenter);
    }

    public static void createAppHeader(SettingsPreferenceFragment fragment, Drawable icon,
            CharSequence label, String pkgName, int uid, Intent externalSettings, boolean titleInCenter) {
        createAppHeader(fragment, icon, label, null, pkgName, uid, 0, externalSettings, titleInCenter);
    }
    ...
    public static void createAppHeader(SettingsPreferenceFragment fragment, Drawable icon,
            CharSequence label, String pkgName, int uid, int tintColorRes, boolean titleInCenter) {
        createAppHeader(fragment, icon, label, null, pkgName, uid, tintColorRes, null, titleInCenter);
    }

    public static void createAppHeader(SettingsPreferenceFragment fragment, Drawable icon,
                                       CharSequence label, CharSequence versionName, String pkgName, int uid, int tintColorRes,
                                       Intent externalSettings, boolean titleInCenter){
        if (titleInCenter){
            ...
            createAppHeader(fragment, icon, label, versionName, pkgName, uid, tintColorRes, externalSettings);
        }

    }
    public static void createAppHeader(SettingsPreferenceFragment fragment, Drawable icon,
                                       CharSequence label, CharSequence versionName, String pkgName, int uid, int tintColorRes
            , boolean titleInCenter) {
        createAppHeader(fragment, icon, label, versionName, pkgName, uid, tintColorRes, null, titleInCenter);
    }
 /*   public static void createAppHeader(SettingsPreferenceFragment fragment, Drawable icon,
    ...
    public static void createAppHeader(SettingsPreferenceFragment fragment, Drawable icon,
                                       CharSequence label, CharSequence versionName, String pkgName, int uid, int tintColorRes,
                                       Intent externalSettings) {
        View view = fragment.setPinnedHeaderView(R.layout.cn_app_header);
        LayoutInflater.from(view.getContext()).inflate(R.layout.widget_text_views,
                (ViewGroup) view.findViewById(android.R.id.widget_frame));

        ImageView iconView = (ImageView) view.findViewById(android.R.id.icon);
        iconView.setImageDrawable(icon);
        // Set application name.
        TextView labelView = (TextView) view.findViewById(android.R.id.title);
        labelView.setText(label);
        // Version number of application
        TextView appVersion = (TextView) view.findViewById(R.id.widget_text1);

        if (!TextUtils.isEmpty(versionName)) {
            appVersion.setSelected(true);
            appVersion.setVisibility(View.VISIBLE);
            appVersion.setText(view.getContext().getString(R.string.version_text,
...
//                tintColorRes, bar, externalSettings);
    ...
    //+++ Asus: util function to set the app info icon tint
    public static void setAppInfoIconTint(SettingsPreferenceFragment fragment,int tintColor) {
        View bar = fragment.getView().findViewById(R.id.pinned_header);
        ImageView appSettings = (ImageView) bar.findViewById(R.id.app_settings);
        if (appSettings != null) {
            appSettings.getDrawable().setTint(tintColor);
        }
    }
    //---
}
