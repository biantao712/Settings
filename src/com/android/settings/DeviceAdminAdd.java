...
public class DeviceAdminAdd extends Activity {
    ...
    protected void onCreate(Bundle icicle) {
        ...
        //+++ suleman
            mRelativeLayout = (RelativeLayout) findViewById(R.id.buttonPane2);
            confirmButton = (Button) findViewById(R.id.action_button);
            cancelButton = (Button) findViewById(R.id.action_button_cancel);
        ...
        mActionButton = (Button) findViewById(R.id.action_button_confirm);
        ...
        if (Utils.isVerizonSKU()) {
            mMdmPolicyManager = new MdmPolicyManager(mDeviceAdmin);
        }
        //END : roy_huang@asus.com
    }
    ...
    void updateInterface() {
        ...
        if (!mRefreshing && !mAddingProfileOwner
            ...
            if (isProfileOwner && isManagedProfile) {
                ...
                //+++ suleman 
                confirmButton.setText("remove_managed_profile_label");
                //--- suleman
                ...
                //+++ suleman
                confirmButton.setText(R.string.security_device_admin_inactive);
                ...
                mTitle.setText(R.string.active_device_admin_msg);
                //---
                if (mUninstalling) {
                    mActionButton.setText(R.string.remove_and_uninstall_device_admin);
                    //+++ suleman
                    confirmButton.setText("remove_and_uninstall_device_admin");
                    //--- suleman 
                } else {
                    mActionButton.setText(R.string.remove_device_admin);
                    //+++ suleman
            ...
            if (mAddingProfileOwner) {
                ...
                mTitle.setText(R.string.profile_owner_add_title);
                //---
            } else {
                //+++ suleman
                //setTitle(getText(R.string.add_device_admin_msg));
                mTitle.setText(R.string.add_device_admin_msg);
                //---
            }
            mActionButton.setText(getText(R.string.add_device_admin));
            //+++ suleman
            confirmButton.setText(R.string.security_device_admin_active);
            //--- suleman
    ...
    private void addDeviceAdminPolicies(boolean showDescription) {
        if (!mAdminPoliciesInitialized) {
            ...
            for (DeviceAdminInfo.PolicyInfo pi : mDeviceAdmin.getUsedPolicies()) {
                ...
                View view = inflater.inflate(R.layout.app_permission_item_old_amax, null);
            ...
            //BEGIN : roy_huang@asus.com
            if (mMdmPolicyManager != null && mMdmPolicyManager.needAppendMdmDeviceAdminPolicy()) {
                Log.d(TAG, "Get target DeviceAdminReceiver and append mdm policies.");

                Set<MdmPolicyInfo> mdmPolicySet = mMdmPolicyManager.getMdmPolicySet();
                if (mdmPolicySet != null && mdmPolicySet.size()>0) {
                    Iterator<MdmPolicyInfo> iter = mdmPolicySet.iterator();
                    while (iter.hasNext()) {
                        MdmPolicyInfo info = iter.next();
                        if (info != null) {
                            View view = AppSecurityPermissions.getPermissionItemView(this, getText(info.getLabelId()),
                                    showDescription ? getText(info.getDescriptionId()) : "", true);
                            mAdminPolicies.addView(view);
                        }
                    }
                }
    ...
    protected void onDestroy() {
        super.onDestroy();
        if (mMdmPolicyManager != null) {
            mMdmPolicyManager = null;
        }
    }

    private static class MdmPolicyManager {
        // -----------------------------------------
        // STATIC FIELDS
        // -----------------------------------------
        private static final String TARGET_DEVICE_ADMIN_RECEIVER = "com.android.email.SecurityPolicy";

        // -----------------------------------------
        // FIELDS
        // -----------------------------------------
        private Set<MdmPolicyInfo> mPolicyInfoSet = new HashSet<MdmPolicyInfo>();
        private boolean mNeedAppendMdmPolicy;

        public MdmPolicyManager(DeviceAdminInfo deviceAdminInfo) {
            mNeedAppendMdmPolicy = checkIfTargetDeviceAdminReceiver(deviceAdminInfo);
            if (mNeedAppendMdmPolicy) {
                mPolicyInfoSet.addAll(createPolicyInfoSet());
            }
        }

        public boolean needAppendMdmDeviceAdminPolicy() {
            return mNeedAppendMdmPolicy;
        }

        public Set<MdmPolicyInfo> getMdmPolicySet() {
            return mPolicyInfoSet;
        }

        private Set<MdmPolicyInfo> createPolicyInfoSet() {
            Set<MdmPolicyInfo> resultSet = new HashSet<MdmPolicyInfo>();

            resultSet.add(new MdmPolicyInfo(com.android.internal.R.string.mdm_policylab_disablebluetooth,
                    com.android.internal.R.string.mdm_policydesc_disablebluetooth));
            resultSet.add(new MdmPolicyInfo(com.android.internal.R.string.mdm_policylab_disablebrowser,
                    com.android.internal.R.string.mdm_policydesc_disablebrowser));
            resultSet.add(new MdmPolicyInfo(com.android.internal.R.string.mdm_policylab_disabletethering,
                    com.android.internal.R.string.mdm_policydesc_disabletethering));
            resultSet.add(new MdmPolicyInfo(com.android.internal.R.string.mdm_policylab_disablesdcard,
                    com.android.internal.R.string.mdm_policydesc_disablesdcard));
            resultSet.add(new MdmPolicyInfo(com.android.internal.R.string.mdm_policylab_disabletextmessaging,
                    com.android.internal.R.string.mdm_policydesc_disabletextmessaging));
            resultSet.add(new MdmPolicyInfo(com.android.internal.R.string.mdm_policylab_disableunsignedapplication,
                    com.android.internal.R.string.mdm_policydesc_disableunsignedapplication));
            resultSet.add(new MdmPolicyInfo(com.android.internal.R.string.mdm_policylab_disableunsignedinstallation,
                    com.android.internal.R.string.mdm_policydesc_disableunsignedinstallation));
            resultSet.add(new MdmPolicyInfo(com.android.internal.R.string.mdm_policylab_disablewifi,
                    com.android.internal.R.string.mdm_policydesc_disablewifi));
            return resultSet;
        }

        private boolean checkIfTargetDeviceAdminReceiver(DeviceAdminInfo deviceAdminInfo) {
            boolean result = false;
            if (deviceAdminInfo != null) {
                ActivityInfo activityInfo = deviceAdminInfo.getActivityInfo();
                if (activityInfo != null && activityInfo.name != null) {
                    result = activityInfo.name.contains(TARGET_DEVICE_ADMIN_RECEIVER);
                }
            }
            return result;
        }
    ...
    private static class MdmPolicyInfo {
        // -----------------------------------------
        // FIELDS
        // -----------------------------------------
        ...
        // TYPES
        // -----------------------------------------
        public MdmPolicyInfo(int labelId, int descriptionId) {
