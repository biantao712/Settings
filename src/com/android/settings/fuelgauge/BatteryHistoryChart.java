...
public class BatteryHistoryChart extends View {
    ...
    void setStats(BatteryStats stats, Intent broadcast) {
        ...
        // BEGIN robert_lcc@asus.com.tw for wifi just watch wifi driver on and off status
    ...
    void finishPaths(int w, int h, int levelh, int startX, int y, Path curLevelPath,
        ...
        if (lastCharging) {
            mChargingPath.lineTo(w, h-mChargingOffset+5);
        }
        if (lastScreenOn) {
            mScreenOnPath.lineTo(w, h-mScreenOnOffset+5);
        }
        if (lastGpsOn) {
            mGpsOnPath.lineTo(w, h-mGpsOnOffset+5);
        }
        if (lastFlashlightOn) {
            mFlashlightOnPath.lineTo(w, h-mFlashlightOnOffset+5);
        }
        if (lastCameraOn) {
            mCameraOnPath.lineTo(w, h-mCameraOnOffset+5);
        }
        if (lastWifiRunning) {
            mWifiRunningPath.lineTo(w, h-mWifiRunningOffset+5);
        }
        if (lastCpuRunning) {
            mCpuRunningPath.lineTo(w, h - mCpuRunningOffset+5);
        }
        if (mHavePhoneSignal) {
    ...
    protected void onSizeChanged(int w, int h, int oldw, int oldh) {
        ...
        mChargingPaint.setStrokeWidth(mLineWidth*2/3);
        mScreenOnPaint.setStrokeWidth(mLineWidth*2/3);
        mGpsOnPaint.setStrokeWidth(mLineWidth*2/3);
        mCameraOnPaint.setStrokeWidth(mLineWidth*2/3);
        mFlashlightOnPaint.setStrokeWidth(mLineWidth*2/3);
        mWifiRunningPaint.setStrokeWidth(mLineWidth*2/3);
        mCpuRunningPaint.setStrokeWidth(mLineWidth*2/3);
                ...
                if (rec.isDeltaData()) {
                    ...
                    if (mLargeMode) {
                        ...
                        if (charging != lastCharging) {
                            if (charging) {
                                mChargingPath.moveTo(x, h-mChargingOffset+5);
                            } else {
                                mChargingPath.lineTo(x, h-mChargingOffset+5);
                            }
                        ...
                        if (screenOn != lastScreenOn) {
                            if (screenOn) {
                                mScreenOnPath.moveTo(x, h-mScreenOnOffset+5);
                            } else {
                                mScreenOnPath.lineTo(x, h-mScreenOnOffset+5);
                            }
                        ...
                        if (gpsOn != lastGpsOn) {
                            if (gpsOn) {
                                mGpsOnPath.moveTo(x, h-mGpsOnOffset+5);
                            } else {
                                mGpsOnPath.lineTo(x, h-mGpsOnOffset+5);
                            }
                            lastGpsOn = gpsOn;
                        }
                        ...
                        if (flashlightOn != lastFlashlightOn) {
                            if (flashlightOn) {
                                mFlashlightOnPath.moveTo(x, h-mFlashlightOnOffset+5);
                            } else {
                                mFlashlightOnPath.lineTo(x, h-mFlashlightOnOffset+5);
                            }
                        ...
                        if (cameraOn != lastCameraOn) {
                            if (cameraOn) {
                                mCameraOnPath.moveTo(x, h-mCameraOnOffset+5);
                            } else {
                                mCameraOnPath.lineTo(x, h-mCameraOnOffset+5);
                            }
                        ...
                        if (wifiRunning != lastWifiRunning) {
                            if (wifiRunning) {
                                mWifiRunningPath.moveTo(x, h-mWifiRunningOffset+5);
                            } else {
                                mWifiRunningPath.lineTo(x, h-mWifiRunningOffset+5);
                            }
                            lastWifiRunning = wifiRunning;
                        }
                        */
                        final boolean wifiRunning =
                            (rec.states2&HistoryItem.STATE2_WIFI_RUNNING_FLAG) != 0;

                        //Log.d(TAG, "onSizeChanged s2=" + rec.states2 + ", wifiRunning=" + wifiRunning + ", lastWifiRunning=" + lastWifiRunning);
                        ...
                        if (cpuRunning != lastCpuRunning) {
                            if (cpuRunning) {
                                mCpuRunningPath.moveTo(x, h - mCpuRunningOffset+5);
                            } else {
                                mCpuRunningPath.lineTo(x, h - mCpuRunningOffset+5);
                            }
    ...
    void drawChart(Canvas canvas, int width, int height) {
        ...
        if (mHavePhoneSignal) {
            ...
            mPhoneSignalChart.draw(canvas, top+10, mLineWidth*2/3);
        }
