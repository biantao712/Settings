...
public class PowerUsageDetail extends PowerUsageBase implements Button.OnClickListener {
    ...
    private static int[] sDrainTypeDesciptions = new int[] {
        R.string.asus_battery_desc_standby,
            ...
            case USER:
            {
                ...
                if (entry.sipper.drainType == BatterySipper.DrainType.APP) {
                    ...
                    if (uid.getUid() != 0) {
                        ...
                        //[TwinApps] {
                        args.putInt(AppInfoBase.ARG_PACKAGE_UID, uid.getUid());
                        //[TwinApps] }
                    }
                }
            }
    ...
    public void onCreate(Bundle icicle) {
        ...
        //[TwinApps] {
        final int packageUid = getArguments().getInt(AppInfoBase.ARG_PACKAGE_UID, 0);
        mUserId = UserHandle.getUserId(packageUid);
    ...
    private void createDetails() {
        ...
        if (mUid >= Process.FIRST_APPLICATION_UID) {
            ...
            //[TwinApps] {
            if (mEnableTwinApps) {
                if (!mUm.getUserInfo(mUserId).isTwinApps()
                        && mApp != null
        ...
        //+++ tim
        setPreferenceGroupChildrenLayout(mControlsParent, R.layout.cnasusres_preference_parent,
                        R.layout.cnasusres_preference_parent_nodivider, true);

        setPreferenceGroupChildrenLayout(mDetailsParent, R.layout.cnasusres_preference_parent_noarrow,
                        R.layout.cnasusres_preference_parent_noarrow_nodivider, true);
        setPreferenceGroupChildrenLayout(mMessagesParent, R.layout.cnasusres_preference_parent_noarrow,
                        R.layout.cnasusres_preference_parent_noarrow_nodivider, true);
        setPreferenceGroupChildrenLayout(mPackagesParent, R.layout.cnasusres_preference_parent_noarrow,
                        R.layout.cnasusres_preference_parent_noarrow_nodivider, true);
        //---
    }
    ...
    private void setupHeader() {
        ...
        //[TwinApps] {
        if (mEnableTwinApps) {
            AppHeader.createAppHeader(this, appIcon, title, null, uid,
                    mDrainType != DrainType.APP ? android.R.color.black : 0, true);
        } else {
        //[TwinApps] }
    ...
    private void startApplicationDetailsActivity() {
        ...
        //[TwinApps] {
        args.putInt(InstalledAppDetails.ARG_PACKAGE_UID, mUid);
        //[TwinApps] }
...
//        pref.setLayoutResource(R.layout.horizontal_preference);
    ...
    private void fillControlsSection(int uid) {
        ...
        PackageInfo pi = null;
        try {
            pi = (packages != null && packages.length > 0) ? pm.getPackageInfo(packages[0], 0) : null;
