...
public final class Utils extends com.android.settingslib.Utils {
    ...
    // BRGIN robert_lcc@asus for Veizon signal strength "perfect"
    /**
    ...
    public static final int[] BADNESS_COLORS = new int[] {
            ...
            0xfffabf2c, 0xff679e37, 0xff0a7f42, 0xff0a7f42
    };
    // END
    ...
    private static final String LAST_FACTORY_RESET_DATA = "last_factory_reset_data";
    private static String mLastFactoryResetTime ;

    private static final String ASUS_LAUNCHER_PACKAGENAME = "com.asus.launcher";
    ...
    public static boolean isMonkeyRunning() {
        return (ActivityManager.isUserAMonkey() || SystemProperties.getBoolean("debug.monkey",false));
    }
    ...
    public static boolean isMultiSimEnabled(Context context) {
        ...
        return telephony != null && telephony.isMultiSimEnabled();
    }
    // --- AMAX @ 20170119 7.1.1 Porting
    ...
    public static Dialog buildGlobalChangeWarningDialog(final Context context, int titleResId,
        ...
        builder.setMessage(R.string.asus_global_change_warning);
    ...
    public static void fragmentStartWithFragment(Context context, String fragmentName, Bundle args,
                                         Fragment resultTo, int resultRequestCode, int titleResId,
                                         CharSequence title, boolean isShortcut) {
        Intent intent = onBuildStartFragmentIntent(context, fragmentName, args,
                null /* titleResPackageName */, titleResId, title, isShortcut);
     ...
     */
    /*For notification*/
    public static Intent onBuildStartFragmentIntentT(Context context, String fragmentName,
            Bundle args, String titleResPackageName, String title,
            boolean isShortcut) {
    ...
    public static int getManagedProfileId(UserManager um, int parentUserId) {
        ...
        for (int profileId : profileIds) {
            if (profileId != parentUserId) {
                 //[TwinApps] {
                 if(TwinAppsUtil.isTwinAppsSupport(um)
                         && profileId == um.getTwinAppsId()) {
   ...
   private static boolean isProfileOf(UserManager um, UserHandle otherUser) {
    ...
    public static Dialog createRemoveConfirmationDialog(Context context, int removingUserId,
        ...
        if (UserHandle.myUserId() == removingUserId) {
            ...
            messageResId = R.string.asus_user_confirm_remove_self_message;
    ...
    public static boolean showSimCardTile(Context context) {
        // +++ rock_huang@20151110: SimCard be handle by Phone team
        String simCard = SystemProperties.get("persist.asus.sim_card", "");
        if (simCard.length() == 0) {
            return false;
        }
        // --- rock_huang@20151110: SimCard be handle by Phone team
    ...
    //tim_hu@asus.com   reject apps to use wifi or mobile, settings in CNMobileManager
    public static Map<Integer, List<String>> getUidInterfaceRules(){
        final INetworkManagementService networkManager = INetworkManagementService.Stub
                .asInterface(ServiceManager.getService(Context.NETWORKMANAGEMENT_SERVICE));
        try {
            return networkManager.getUidInterfaceRules();
        } catch (RemoteException e) {
            return null;
        }
    }
    ...
    //+++ Jason_Uang
    /**
     * Return whether the device is a Padfone and has plug-in a station.
     * @param context
     * @return
     */
    public static boolean isOnStationMode(Context context) {
        PackageManager pm = context.getPackageManager();
        final boolean hasPadfoneStationFeatureEC = pm.hasSystemFeature(PackageManager.FEATURE_ASUS_FIRMWARE_PADFONE_STATION_PAD_EC);
        final boolean hasPadfoneStationFeatureTouch = pm.hasSystemFeature(PackageManager.FEATURE_ASUS_FIRMWARE_PADFONE_STATION_TOUCH);
        final boolean isOnTabletMode = context.getResources().getBoolean(R.bool.config_systembar_tablet);
        return ((hasPadfoneStationFeatureEC || hasPadfoneStationFeatureTouch) && isOnTabletMode);
    }
    //+++
    ...
    public static boolean isCNSKU(){
        ...
        String SKU2 = android.os.SystemProperties.get("ro.config.versatility", "");
        return (SKU.toLowerCase().startsWith("cn_") || SKU2.toLowerCase().startsWith("cn"));
    }

    public static boolean isCTCC(){
        String CID = SystemProperties.get("ro.config.CID").toLowerCase();
        return (CID.toLowerCase().startsWith("ctcc") );
    }

    private static boolean isLessThan1GRam(Context context) {
        ActivityManager actManager = (ActivityManager) context.getSystemService(Context.ACTIVITY_SERVICE);
        MemoryInfo memInfo = new ActivityManager.MemoryInfo();
        actManager.getMemoryInfo(memInfo);
        return memInfo.totalMem < 1 << 30;
    }
    /**
     * Disable lockscreen wallpaper settings on Launcher<BR>
     *  if device is later than LOLLIPOP and the ram size is less than 1G
     *  or sku is IQY
     */
    public static boolean disableLockWallpaperOptionsOnLauncher(Context context) {
        return (Build.VERSION.SDK_INT >= Build.VERSION_CODES.LOLLIPOP
                  && isLessThan1GRam(context))
                  || isIqySku();
    }

    public static boolean isIqySku() {
        return isProductNameStartsWithLowerCase("iqy");
    }

    private static boolean isProductNameStartsWithLowerCase(String startLowerCaseChars) {
        String productName = android.os.SystemProperties.get("ro.product.name", "");
        return TextUtils.isEmpty(productName) ?
                false : productName.trim().toLowerCase().startsWith(startLowerCaseChars);
    }
    ...
    public static boolean isVerizon() {
        ...
        Log.d("VerizonUtil", "isVerizon(): ret = " + ret);
        return ret;
    }

    public static String getCIDEncode(Context context){
        String CID = android.os.SystemProperties.get("ro.config.CID", "");
        if (CID == "") return "00000";
        String[] cid_list = context.getResources().getStringArray(R.array.cid_table);
        String[] cid_code = context.getResources().getStringArray(R.array.cid_code);
        Map<String,String> map = new HashMap<String,String>();
        for(int i=0; i<cid_list.length; i++){
            map.put(cid_list[i], cid_code[i]);
        }
        return map.containsKey(CID.toUpperCase()) ? map.get(CID.toUpperCase()) : "00000";
    }

    public static String getCountryEncode(Context context) {
        String country = android.os.SystemProperties.get(
                "ro.config.versatility", "");
        if (country == "")
        return "00000";
        String[] country_list = context.getResources().getStringArray(
                R.array.country_table);
        String[] country_code = context.getResources().getStringArray(
                R.array.country_code);
        Map<String, String> map = new HashMap<String, String>();
        for (int i = 0; i < country_code.length; i++) {
            map.put(country_list[i], country_code[i]);
        }
        return map.containsKey(country.toUpperCase()) ? map.get(country.toUpperCase()) : "00000";
    }

    public static String getSKUEncode() {
        String SKU = android.os.SystemProperties.get("ro.product.name", "");
        if (SKU == "")
        return "000";
        if (SKU.toUpperCase().startsWith("WW")) {
            return "001";
        } else if (SKU.toUpperCase().startsWith("JP")) {
            return "002";
        } else if (SKU.toUpperCase().startsWith("CN")){
            return "003";
        }
        return "000";
    }
    ...
    public static boolean isAdminUser() {
        return UserHandle.USER_SYSTEM == ActivityManager.getCurrentUser();
    }

    //+++ Asus For 1st part app restrict support
    /**
     * Return whether the package is asus defined sticky apps
     * @param pkg
     * @return
     */
    public static boolean isAsusStickyApp(PackageInfo pkg) {
        return (pkg.applicationInfo.enabled && (pkg.applicationInfo.flagsAsus & ApplicationInfo.FLAG_STICKY_PRELOADED_APP) != 0);
    }
    //---

    //+++ Asus: for 2nd party Apps protecttion
    /**
     * Method to detect the 2nd Asus defined 2nd party apps
     * @param pkg
     * @return boolean to indicate whether the input package is belonging to Asus 2nd part apps
     */
    public static boolean isProtectedSecApp(PackageInfo pkg) {
        return ((!pkg.applicationInfo.enabled) | (pkg.applicationInfo.flagsAsus & ApplicationInfo.FLAG_SEC_PART_APP) != 0);
    }
    //---

    // BEGIN : roy_huang@asus.com
    public static boolean isCustomizedDeviceAdminActive(Context context) {
        boolean result = false;
        DevicePolicyManager dpm = (DevicePolicyManager) context.getSystemService(Context.DEVICE_POLICY_SERVICE);
        result = dpm.isAdminActive(new ComponentName("com.asus.email", "com.android.email.SecurityPolicy$PolicyAdmin"));
        Log.d(TAG, "isCustomizedDeviceAdminActive : " + result);
        return result;
    }
    ...
    // +++ Kevin_Chiou@20151217: Verizon VZ_REQ_UI_15743
    public static String getFirstInstallTime(Context context) {
        try {
            Calendar calendar = Calendar.getInstance();
            calendar.setTimeInMillis(context.getPackageManager().getPackageInfo("com.android.settings", 0).firstInstallTime);
            Log.e("DAPON", "Date : " + DateFormat.getDateTimeInstance().format(calendar.getTime()));
    ...
    public static void parseLastFactoryTime(Context context) {
        String line;
        ...
        mLastFactoryResetTime = stringBuilder.toString();
        android.provider.Settings.System.putString(context.getContentResolver(),
                LAST_FACTORY_RESET_DATA, mLastFactoryResetTime);
    }

    public static String getLastFactoryTime() {
        return mLastFactoryResetTime;
    }

    // +++ Kevin_Chiou Verizon VZ_REQ_UI_15743
    public static boolean isLastFactoryFileExist() {
        File f = new File(FACTORY_PARTITION_ROOT_PATH+"/"+FACTORY_RESET_TIME_FILE_NAME);
        Log.d(TAG, "is " +FACTORY_PARTITION_ROOT_PATH+"/"+FACTORY_RESET_TIME_FILE_NAME
                + " exist: " + String.valueOf(f.exists()));
        return f.exists();
    }
    // --- Kevin_Chiou@20151217: Verizon VZ_REQ_UI_15743
    ...
    public static boolean isAudioWizardEntryEnabled() {
        boolean result = false;
        if (SystemProperties.getInt("ro.asus.aw.settingentry", 0) == 1) {
            result = true;
        }
        return result;
    }

    // +++ Jacob_Ke, Do not show overlay option for AsusLauncher above android 7.1.1
    public static boolean shouldShowOverlayOption(String packageName) {
        return !(ASUS_LAUNCHER_PACKAGENAME.equals(packageName) && ATLEAST_N_MR1);
    }
    // --- Jacob_ke

    public static boolean isEasyLauncher(Context context){
