...
public class DashboardAdapter extends RecyclerView.Adapter<DashboardAdapter.DashboardItemHolder>
    ...
    private Map<Tile, AbstractEnabler> mToggles = new HashMap<>();
    ...
    //
    //  - If this icon of package is coming from outside Settings, and we don't tint it to
    //    match Settings' color, please add it to this white list.
    //
    private static final String[] NO_TINTED_ICON_PACKAGENAME_LIST = {
        AlwaysOnUtils.getSettingsPackageName(),  // [Always-on Panel][AlwaysOn]
    };
    //
        ...
        for (int i = 0; i < categories.size(); i++) {
            for (int j = 0; j < categories.get(i).tiles.size(); j++) {
                ...
                if (!mContext.getPackageName().equals(
                    ...
                    // Added by Mingszu Liang, 2016.09.01. - BEGIN.
                    //
                    // If this package is in the white list, we don't tint its icon.
                    if (ArrayUtils.contains(NO_TINTED_ICON_PACKAGENAME_LIST,
                                                tile.intent.getComponent().getPackageName())) {
                        continue;
                    }
                    //
                    // Added by Mingszu Liang, 2016.09.01. - END.
    ...
    public void onResume() {
        for(AbstractEnabler enabler: mToggles.values()){
            enabler.resume();
        }
    }
...
//        countItem(null, R.layout.dashboard_spacer, true, NS_SPACER);
        ...
        for (int i = 0; mCategories != null && i < mCategories.size(); i++) {
            ...
            countItem(null, R.layout.category_divider, true, NS_SPACER);
    ...
    public void onBindViewHolder(DashboardItemHolder holder, int position) {
        switch (mTypes.get(position)) {
            ...
            case R.layout.suggestion_tile:
                ...
                holder.itemView.findViewById(R.id.suggestion_remove).setOnClickListener(
                        new View.OnClickListener() {
                            @Override
                            public void onClick(View v) {
    ...
    private void onBindSuggestionHeader(final DashboardItemHolder holder) {
        ...
        holder.title.setText(mContext.getString(R.string.suggestions_title, mSuggestions.size())+ "(" + mSuggestions.size() + ")" );
    ...
    private void countToggle(Tile tile){
        String className = tile.intent.getComponent().getClassName();
        if(className.equals(Settings.LocationSettingsActivity.class.getName())){
            if(mToggles.containsKey(tile)) return;
            LocationEnabler locationEnabler = new LocationEnabler(mContext);
            mToggles.put(tile, locationEnabler);
        }
    }

    private void setEnabler(DashboardItemHolder holder, Tile tile){
        if (mToggles.containsKey(tile)) {
            holder.switchWidget.setVisibility(View.VISIBLE);
            mToggles.get(tile).setSwitch(holder.switchWidget);
        } else {
    ...
    public static class DashboardItemHolder extends RecyclerView.ViewHolder {
        ...
        public DashboardItemHolder(View itemView) {
            ...
            switchWidget = (Switch)itemView.findViewById(android.R.id.switch_widget);
