...
public class RunningState {
    ...
    static class ProcessItem extends BaseItem {
        ...
        boolean updateService(Context context, ActivityManager.RunningServiceInfo service) {
            ...
            if (!isExistUser(
                    (UserManager) context.getApplicationContext()
                            .getSystemService(Context.USER_SERVICE), UserHandle
                            .getUserId(service.uid)))
                return changed;
    ...
    private boolean update(Context context, ActivityManager am) {
        ...
        if (changed) {
            ...
            for (int i=0; i<sortedProcesses.size(); i++) {
                ...
                if (isExistUser(mUm,mergedItem.mUserId) && mergedItem.mUserId != mMyUserId) {
    ...
    static private boolean isExistUser(UserManager um ,int uid){
        List<UserInfo> userList = um.getUsers(true);
        boolean isExist = false;
        for (int useri = 0; useri < userList.size(); useri++) {
            isExist = uid == userList.get(useri).id;
