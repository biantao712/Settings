...
public class ManageApplications extends InstrumentedFragment
    ...
    public static final AppFilter[] FILTERS = new AppFilter[]{
            ...
            AppStateOverlayBridge.FILTER_SYSTEM_ALERT_WINDOW_NO_ZENUI,   // Apps that can draw overlays
    ...
    public void onCreate(Bundle savedInstanceState) {
        ...
        Log.d(TAG, "ManageApplications mListType = " + mListType);
        if(mListType == LIST_TYPE_HIGH_POWER){
            args.putString(EXTRA_CLASSNAME, className);
            if (getActivity() instanceof SettingsActivity) {
                ((SettingsActivity) getActivity()).startPreferencePanel(
                        CNSManageApplications.class.getCanonicalName(), args,
                        R.string.high_power_apps, null, this, 0);
            }
    ...
    private void createHeader() {
        ...
        if (mListType == LIST_TYPE_MAIN) {
            ...
            for(UserHandle userHandle : userProfiles) {
                if(UserManager.get(getActivity()).getUserInfo(userHandle.getIdentifier()).isManagedProfile()) {
                    mFilterAdapter.enableFilter(FILTER_APPS_PERSONAL);
                    mFilterAdapter.enableFilter(FILTER_APPS_WORK);
                    break;
                }
...
/*    @Override
    ...
    void updateButtonBar() {
        ...
        if (mListType == LIST_TYPE_MAIN || mListType == LIST_TYPE_NOTIFICATION){
            (mButtonBar.getButton(1)).setVisibility(View.VISIBLE);
        } else{
            (mButtonBar.getButton(1)).setVisibility(View.GONE);
        }

        if (mListType != LIST_TYPE_HIGH_POWER){
            (mButtonBar.getButton(2)).setVisibility(View.VISIBLE);
            if (!mShowSystem){
                (mButtonBar.getButton(2)).setButton(2, R.drawable.asus_ic_wifisettings_menu_configure,
                        getActivity().getResources().getString(R.string.menu_show_system));
            }else{
                (mButtonBar.getButton(2)).setButton(2, R.drawable.asus_ic_wifisettings_menu_configure,
                        getActivity().getResources().getString(R.string.menu_hide_system));
            }
        } else{
            (mButtonBar.getButton(2)).setVisibility(View.GONE);
        }
        if (mListType == LIST_TYPE_STORAGE){
            (mButtonBar.getButton(3)).setVisibility(View.VISIBLE);
        }else{
            (mButtonBar.getButton(3)).setVisibility(View.GONE);
        }
...
//        updateOptionsMenu();
    ...
    static class ApplicationsAdapter extends BaseAdapter implements Filterable,
        ...
        private Filter mFilter = new Filter() {
            ...
            protected void publishResults(CharSequence constraint, FilterResults results) {
                ...
                //Kevin_Chiou Verizon hide Verizon Store Demo Mode and llk
                removellkandVerizonStoreDemoMode();
        ...
        //Kevin_Chiou++ Verizon hide Verizon Store Demo Mode and llk
        private void removellkandVerizonStoreDemoMode(){
            if(mEntries != null && !mEntries.isEmpty()){
                for(int i=0;i< mEntries.size();i++){
                    ApplicationsState.AppEntry entry = mEntries.get(i);
                    if(entry != null){
                        String pkg = entry.info.packageName;
                        boolean bStoreDemoHide = ("com.customermobile.preload.vzw").equals(pkg);
                        boolean bllkHide = ("com.verizon.llkagent").equals(pkg);
                        if (bStoreDemoHide || bllkHide) {
                            mEntries.remove(i);
                            i--;
                        }
                    }
    ...
    private void initButtonBar(){
        ...
        mButtonBar = ((SettingsActivity)getActivity()).getButtonBar();
        if(mButtonBar != null) {
            mButtonBar.setVisibility(View.VISIBLE);
            mButtonBar.addButton(1, R.drawable.asus_ic_wifisettings_menu_configure,
                    getActivity().getResources().getString(R.string.advanced_apps));
            ...
            mButtonBar.addButton(2, R.drawable.asus_ic_wifisettings_menu_configure,
                    getActivity().getResources().getString(R.string.menu_show_system));
            ...
            mButtonBar.addButton(3, R.drawable.cn_application_sort_button,
            ...
            mButtonBar.addButton(4, R.drawable.cn_application_reset_button,
    ...
    private void onAdvanceButtonClick(){
        if (mListType == LIST_TYPE_NOTIFICATION) {
            ((SettingsActivity) getActivity()).startPreferencePanel(
                    ConfigureNotificationSettings.class.getName(), null,
                    R.string.configure_notification_settings, null, this, ADVANCED_SETTINGS);
