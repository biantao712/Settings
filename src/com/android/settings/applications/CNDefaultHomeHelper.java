...
public class CNDefaultHomeHelper extends CNDefaultAppHelperBase{

    private final ArrayList<ComponentName> mAllHomeComponents = new ArrayList<>();
    private final IntentFilter mHomeFilter;

    private static CNDefaultHomeHelper mInstance;
    private CNDefaultHomeHelper(Context context){
        super(context);
    ...
    public static CNDefaultHomeHelper getInstance(Context context){
        if (mInstance == null){
            mInstance = new CNDefaultHomeHelper(context);
        }
        return mInstance;
    }
    ...
    public void setHomeValue(String value) {
        if (value != null) {
            ...
            mContext.getPackageManager().replacePreferredActivity(mHomeFilter,
                    IntentFilter.MATCH_CATEGORY_EMPTY,
                    mAllHomeComponents.toArray(new ComponentName[0]), component);
        }
    }
    public void setToDefault(){
        for (int i = 0; i<mAllHomeComponents.size(); i++) {
            mContext.getPackageManager().clearPackagePreferredActivities(mAllHomeComponents.get(i).getPackageName());
        }
    }
    public void refreshHomeOptions() {
        final String myPkg = mContext.getPackageName();
        ...
        for (int i = 0; i < homeActivities.size(); i++) {
            ...
            if (mustSupportManagedProfile && !launcherHasManagedProfilesFeature(candidate, pm)) {
                summaries.add(mContext.getString(R.string.home_work_profile_not_supported));
            } else {
                summaries.add(null);
            }
        }
    ...
    private boolean hasManagedProfile() {
        ...
        List<UserInfo> profiles = userManager.getProfiles(mContext.getUserId());
