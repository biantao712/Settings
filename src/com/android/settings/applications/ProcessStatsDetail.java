...
public class ProcessStatsDetail extends SettingsPreferenceFragment {
    ...
    private AsusButtonBarButton mStopButton;
    ...
    public void onCreate(Bundle icicle) {
        ...
        mLabel = args.getString("EXTRA_LABEL");
        //[TwinApps] {
        if (mEnableTwinApps && mTwinAppsId != -1){
            mApp.retrieveUiDataAsUser(getActivity(), mPm);
        } else {
        //[TwinApps] }
    ...
    public void onViewCreated(View view, Bundle savedInstanceState) {
        ...
        if(mEnableTwinApps){
            final Drawable icon;
            if(mTwinAppsId == UserHandle.getUserId(mApp.mUiTargetApp.uid)){
                icon = mPm.getUserBadgedIcon(mApp.mUiTargetApp.loadIcon(mPm), new UserHandle(mTwinAppsId));
            } else {
                icon = mApp.mUiTargetApp.loadIcon(mPm);
            }
            AppHeader.createAppHeader(this, icon, mLabel!=null?mLabel:mApp.mUiLabel, mApp.mPackage, mApp.mUiTargetApp.uid, true);
        } else {
        //[TwinApps] }
            AppHeader.createAppHeader(this,
                    mApp.mUiTargetApp != null ? mApp.mUiTargetApp.loadIcon(mPm) : new ColorDrawable(0),
                    mLabel!=null?mLabel:mApp.mUiLabel, mApp.mPackage, mApp.mUiTargetApp.uid, true);
        //[TwinApps] {
        }
        //[TwinApps] }
        initButtonBar();
    ...
    private void createDetails() {
        ...
        StorageSummaryPreference summaryPreference = (StorageSummaryPreference) findPreference(KEY_DETAILS_HEADER);
...
//        summaryPreference.setUnits(usedResult.units);
        ...
        summaryPreference.setPercent((int) (avgRatio*100));
        ...
        summaryPreference.setTitle(title);
...
/*
    ...
    private void fillProcessesSection() {
        ...
        for (int ie = 0; ie < entries.size(); ie++) {
            ...
            processPref.setLayoutResource(R.layout.cnasusres_preference_parent_noarrow);
    ...
    private void killProcesses() {
        ...
        for (int i=0; i< mApp.mEntries.size(); i++) {
            ...
            for (int j=0; j<ent.mPackages.size(); j++) {
                //[TwinApps] {
                if(mEnableTwinApps){
                    am.forceStopPackageAsUser(ent.mPackages.get(j), UserHandle.getUserId(ent.mUid));
                } else {
                //[TwinApps] }
    ...
    private void checkForceStop() {
        ...
        if (mApp.mEntries.get(0).mUid < Process.FIRST_APPLICATION_UID) {
            ...
            Log.d("blenda", "mButtonBar.setVisibility(View.GONE)");
            return;
        }
        ...
        for (int i=0; i< mApp.mEntries.size(); i++) {
            ...
            for (int j=0; j<ent.mPackages.size(); j++) {
                ...
                if (mDpm.packageHasActiveAdmins(pkg)) {
                    ...
                    Log.d("blenda", "mStopButton.setEnabled(false)");
                ...
                if (mEnableTwinApps) {
                    try {
                        ...
                        final PackageInfo info = ipm.getPackageInfo(pkg, 0, UserHandle.getUserId(ent.mUid));
                        if (info != null && (info.applicationInfo.flags & ApplicationInfo.FLAG_STOPPED) == 0) {
                            isStarted = true;
                        }
    ...
    private void initButtonBar(){
        ...
        if(mButtonBar != null) {
            mButtonBar.setVisibility(View.VISIBLE);
            mButtonBar.addButton(1, R.drawable.cn_forbid_button,
...
//                    setDialog();
        ...
        mStopButton = mButtonBar.getButton(1);
    }
}
