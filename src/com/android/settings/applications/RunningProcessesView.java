...
public class RunningProcessesView extends FrameLayout
    ...
    public static class ViewHolder {
        ...
        public RelativeLayout mContentLine2;
        ...
        public ViewHolder(View v) {
            ...
            mContentLine2 = (RelativeLayout)v.findViewById(R.id.content_line2);
            v.setTag(this);
        }
        ...
        public ActiveItem bind(RunningState state, RunningState.BaseItem item,
            ...
            synchronized (state.mLock) {
                ...
                if (item.mBackground) {
                    ...
                    mContentLine2.setVisibility(View.GONE);
                } else {
                    description.setText(item.mDescription);
                    mContentLine2.setVisibility(View.VISIBLE);
                }
    ...
    class ServiceListAdapter extends BaseAdapter {
        ...
        //Ken1_Yu, default show cache process memory usage
        ...
        void refreshItems() {
            ...
            if (null != newItems && !newItems.isEmpty()) {
                if (TwinAppsUtil.isTwinAppsSupport(getContext())) {
                    ...
                    final PackageManager pm = (PackageManager) getContext().getPackageManager();
                    int twinAppsId = um.getTwinAppsId();
                    if (twinAppsId != -1) {
                        for (int i=0; i< newItems.size(); i++) {
                            RunningState.MergedItem entry = newItems.get(i);
                            if (entry.mUserId == twinAppsId) {
                                if (entry.mPackageInfo != null) {
                                    try {
                                        final ApplicationInfo applicationInfo = pm.getApplicationInfo(entry.mPackageInfo.packageName, entry.mUserId);
                                        if ((applicationInfo.isSystemApp() || applicationInfo.isUpdatedSystemApp())
                                                /*&& !entry.hasLauncherEntry*/) {
                                            newItems.remove(i);
                                            i--;
                                        } else if (("com.android.settings").equals(entry.mPackageInfo.packageName)) {
                                            newItems.remove(i);
                                            i--;
                                        }
                            ...
                            } else if (entry.mPackageInfo != null && ("com.asus.twinapps").equals(entry.mPackageInfo.packageName)) {
                                newItems.remove(i);
                                i--;
                            }
                        }
                    } else {
                        //always remove twinapp
                        for (int i=0; i< newItems.size(); i++) {
                            RunningState.MergedItem entry = newItems.get(i);
                            if (entry.mPackageInfo != null && ("com.asus.twinapps").equals(entry.mPackageInfo.packageName)) {
                                newItems.remove(i);
                                i--;
                            }
                        }
        ...
        public View newView(ViewGroup parent) {
            ...
            if (mInAppManager){
                if (mShowBackground){
                    v = mInflater.inflate(R.layout.running_processes_item_am_bg, parent, false);
                }else
                    v = mInflater.inflate(R.layout.running_processes_item_am, parent, false);
            }else{
                v = mInflater.inflate(R.layout.running_processes_item, parent, false);
            }
            new ViewHolder(v);
            return v;
        }
    ...
    public void doCreate() {
        ...
        mColorBar.setColors(context.getColor(R.color.running_processes_system_ram),
                /*Utils.getColorAccent(context)*/context.getColor(R.color.running_processes_apps_ram),
    ...
    private boolean mInAppManager = false;
    public void doCreate(boolean hasHeader, boolean inAppManager) {
