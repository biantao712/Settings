 ...
 * intent.
 */
public class CNNotifyApplications extends InstrumentedFragment
        implements OnItemClickListener{

    static final String TAG = "CNNotifyApplications_NF";
    static final boolean DEBUG = Log.isLoggable(TAG, Log.DEBUG);
    private int notifyType = EXTRA_APPLICATIONS_TYPE_NOTIFY;
    // Intent extras.
    ...
    private int type;
    private CNNotifySync mCNManageAppsSync;
    ...
    public void onCreate(Bundle savedInstanceState) {
        ...
        if(type == EXTRA_APPLICATIONS_TYPE_NOTIFY_IMPORTANCE)
        {
            mListType = LIST_TYPE_NOTIFICATION;
            notifyType = EXTRA_APPLICATIONS_TYPE_NOTIFY_IMPORTANCE;
            mNotifBackend = new NotificationBackend();
        }else if(type == EXTRA_APPLICATIONS_TYPE_NOTIFY_HEADSUP)
        {
            mListType = LIST_TYPE_NOTIFICATION;
            notifyType = EXTRA_APPLICATIONS_TYPE_NOTIFY_HEADSUP;
            mNotifBackend = new NotificationBackend();
        }else if(type == EXTRA_APPLICATIONS_TYPE_NOTIFY_ALLOWED)
        {
            mListType = LIST_TYPE_NOTIFICATION;
            notifyType = EXTRA_APPLICATIONS_TYPE_NOTIFY_ALLOWED;
            mNotifBackend = new NotificationBackend();
        }else if(type == EXTRA_APPLICATIONS_TYPE_NOTIFY)
        {
            mListType = LIST_TYPE_NOTIFICATION;
            notifyType = EXTRA_APPLICATIONS_TYPE_NOTIFY;
            mNotifBackend = new NotificationBackend();
        }else if (type == EXTRA_APPLICATIONS_TYPE_INSTALLED){
...
//        mSortOrder = SORT_ORDER_SIZE;
        ...
        if (savedInstanceState != null) {
            ...
            Log.d("blenda", "onCreate， get savedInstanceState, mListType: "+mListType);
        ...
        mCNManageAppsSync = CNNotifySync.getInstance();
        setInterface();
    }
...
//        }
        ...
        mInflater = inflater;
        mRootView = inflater.inflate(R.layout.manage_applications_notify, null);
        ...
        if (mListContainer != null) {
            ...
            mApplications.setNotifyType(notifyType);
            if (savedInstanceState != null) {
...
//        }
    ...
    // utility method used to start sub activity and set Title of the apk name
    private void startApplicationDetailsActivity(String applabel) {
        switch (mListType) {
            case LIST_TYPE_NOTIFICATION:
    ...
    private void startApplicationDetailsActivity(int applabel) {
        switch (mListType) {
            case LIST_TYPE_NOTIFICATION:
                startAppInfoFragment(CNAppNotifySettings.class,
                        applabel);
    ...
    private void startAppInfoFragment(Class<?> fragment, String label) {
        AppInfoBase.startAppInfoFragmentT(fragment, label, mCurrentPkgName, mCurrentUid, this,
    ...
    public void onItemClick(AdapterView<?> parent, View view, int position, long id) {
        if (mApplications != null && mApplications.getCount() > position) {
            ...
            String label = (String)getContext().getPackageManager().getApplicationLabel(entry.info);
            startApplicationDetailsActivity(label);
            //startApplicationDetailsActivity(entry.info.labelRes);
        }
    }
    ...
    static class FilterSpinnerAdapter extends ArrayAdapter<CharSequence> {
        ...
        private final CNNotifyApplications mManageApplications;
        ...
        public FilterSpinnerAdapter(CNNotifyApplications manageApplications) {
    ...
    static class ApplicationsAdapter extends BaseAdapter implements Filterable,
        ...
        private int notify = EXTRA_APPLICATIONS_TYPE_NOTIFY;
        ...
        private Filter mFilter = new Filter() {
            ...
            protected void publishResults(CharSequence constraint, FilterResults results) {
                ...
                //removeLiveDemo(); // moved to check Notify List
                // -[AMAX01][LiveDemo] TJ Tsai, 2016.08.22
                //[TwinApps] {
                removeTwinApps();
                //[TwinApps] }
                //Mark_Xiao
                checkNotifyList(notify);
                rebuildSections();
                notifyDataSetChanged();
            }
        };

        public ApplicationsAdapter(ApplicationsState state, CNNotifyApplications manageApplications,
                                   int filterMode) {
            mState = state;
            ...
            if (mManageApplications.mListType == LIST_TYPE_NOTIFICATION) {
                mNotifBackend = manageApplications.mNotifBackend;
        ...
        private void checkNotifyList(int type)
        {
            if (mEntries != null && !mEntries.isEmpty()) {
                for (int i = 0; i < mEntries.size(); i++) {
                    ...
                    if (entry != null) {
                        ...
                        int uid = entry.info.uid;
                        if (("com.asus.livedemo").equals(pkg) || ("com.asus.livedemoservice").equals(pkg)) {
                            bHide = true;
                        }else if(type == EXTRA_APPLICATIONS_TYPE_NOTIFY)
                        {
                            bHide = false;
                        }else if(type == EXTRA_APPLICATIONS_TYPE_NOTIFY_IMPORTANCE){
                            if(mNotifBackend.getStyleImportance(pkg,uid)==1){
                                bHide = false;
                            }else{
                                bHide = true;
                            }
                            if(!bHide)
                            {
                                if(mNotifBackend.getAllowed(pkg,uid)!=1)
                                {
                                    bHide = true;
                                }
                            }
                        }else if(type == EXTRA_APPLICATIONS_TYPE_NOTIFY_HEADSUP){
                            if(mNotifBackend.getStyleHeadsUp(pkg,uid)==1){
                                bHide = false;
                            }else{
                                bHide = true;
                            }
                            ...
                            if(mNotifBackend.getAllowed(pkg,uid)==0){
                                bHide = false;
                            }else{
                                bHide = true;
                            }
...
//            mManageApplications.setHasDisabled(mState.haveDisabledApps());
        ...
        public void onPackageSizeChanged(String packageName) {
            for (int i = 0; i < mActive.size(); i++) {
                CNAppViewHolder holder = (CNAppViewHolder) mActive.get(i).getTag();
        ...
        public View getView(int position, View convertView, ViewGroup parent) {
            ...
            CNAppViewHolder holder = CNAppViewHolder.createOrRecycle(mManageApplications.mInflater,
                    convertView);
        ...
        private void updateSummary(CNAppViewHolder holder) {
            ...
            if (listening) {
                new AppCounter(mContext) {
                    ...
                    protected void onCountComplete(int num) {
                        mLoader.setSummary(CNNotifyApplications.SummaryProvider.this,
