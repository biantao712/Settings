...
public class ProcStatsPackageEntry implements Parcelable {
    ...
    //[TwinApps] {
    public void retrieveUiDataAsUser(Context context, PackageManager pm) {
        mUiTargetApp = null;
        mUiLabel = mPackage;
        final int userId = mEntries.isEmpty() ? -1 : mEntries.get(0).mUid;
        // Only one app associated with this process.
        try {
            if ("os".equals(mPackage)) {
                mUiTargetApp = pm.getApplicationInfo("android",
                ...
                if (userId != -1) {
                    try {
                        ...
                        final PackageInfo mPackageInfo = ipm.getPackageInfo(mPackage,
                                PackageManager.GET_DISABLED_COMPONENTS |
                                        PackageManager.GET_DISABLED_UNTIL_USED_COMPONENTS |
                                        PackageManager.GET_UNINSTALLED_PACKAGES, UserHandle.getUserId(userId));
                        mUiTargetApp = mPackageInfo==null ? null : mPackageInfo.applicationInfo;
